...
secret_file="$RUNNER_TEMP/pr552-v12-vercel-bypass-secret"
cleanup_completed=0
revoke_completed=0
...
revoke_bypass() {
  ...
  request_file="$RUNNER_TEMP/pr552-v12-vercel-bypass-revoke-request.json"
  response_file="$RUNNER_TEMP/pr552-v12-vercel-bypass-revoke-response.json"
...
on_exit() {
  ...
  rm -f "$secret_file" \
    "$RUNNER_TEMP/pr552-v12-vercel-bypass-generate-request.json" \
    "$RUNNER_TEMP/pr552-v12-vercel-bypass-generate-response.json" \
    "$RUNNER_TEMP/pr552-v12-vercel-bypass-revoke-request.json" \
    "$RUNNER_TEMP/pr552-v12-vercel-bypass-revoke-response.json"
  exit "$status"
}
trap 'on_exit $?' EXIT
...
request_file="$RUNNER_TEMP/pr552-v12-vercel-bypass-generate-request.json"
response_file="$RUNNER_TEMP/pr552-v12-vercel-bypass-generate-response.json"
jq -n \
  --arg secret "$secret" \
  --arg note "PR552 exact Preview MFA QA v12 run ${GITHUB_RUN_ID:-unknown}" \
