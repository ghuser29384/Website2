...
# Generate the synthetic identity before any temporary Vercel mutation so every
# always() cleanup step has an exact user scope even if environment creation fails.
...
# Replace the rejected REST payload with Vercel's supported branch-scoped CLI
# flow, then prove the resulting sensitive Preview variable through API readback.
env_index = source.index(env_start)
env_end_index = source.index(env_end, env_index)
new_env_block = r'''      - name: Create exact branch-scoped sensitive Vercel Preview variable
        id: env_create
        shell: bash
        run: |
          set -euo pipefail
...
# CLI-created branch variables need not carry the old REST comment. Cleanup is
# still exact because the workflow first proves that no matching variable exists,
# then requires exactly one matching key+branch variable after creation.
cleanup_start = """            jq --arg branch "$CONTROLLER_BRANCH" --arg comment "$label" '[
"""
...
new_cleanup = """            jq --arg branch "$CONTROLLER_BRANCH" '[
              ((.envs // .)[])
              | select(.key == "SUPABASE_SERVICE_ROLE_KEY")
              | select((.gitBranch // "") == $branch)
...
# Always emit valid JSON evidence when a resource was never created.
replacements = {
