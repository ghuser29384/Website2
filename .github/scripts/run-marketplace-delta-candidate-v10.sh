...
cp .github/scripts/patch-marketplace-delta-v10.py \
  "$RUNNER_TEMP/patch-marketplace-delta-v10.py"
python3 "$RUNNER_TEMP/patch-marketplace-delta-v10.py" \
...
old = 'bash "$runner"\n'
new = '''python3 "$RUNNER_TEMP/patch-marketplace-delta-v10.py" --runner "$runner"
bash "$runner"
'''
if source.count(old) != 1:
