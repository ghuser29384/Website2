...
async function createSession({ label, viewport, user }) {
  ...
  const diagnostics = {
    label,
    consoleErrors: [],
    ignoredRateLimitedBeaconErrors: [],
    rateLimitedFunnelEvents: [],
    pageErrors: [],
    failedRequests: [],
    badResponses: [],
  };
  ...
  page.on("console", (message) => {
    ...
    if (text === "Failed to load resource: the server responded with a status of 429 (Too Many Requests)") {
      diagnostics.ignoredRateLimitedBeaconErrors.push(text);
      return;
    }
  ...
  page.on("response", (response) => {
    ...
    if (
      ...
      new URL(response.url()).pathname === "/api/funnel-events" &&
      response.status() === 429 &&
      request.resourceType() === "ping"
    ) {
      diagnostics.rateLimitedFunnelEvents.push({
        method: request.method(),
        resourceType: request.resourceType(),
...
function isSyntheticQaEmail(email) {
  ...
  return normalized.startsWith(USER_PREFIX) && normalized.endsWith("@example.com");
}

async function listSyntheticQaUsers() {
  const users = [];
  for (let page = 1; page <= 30; page += 1) {
    const { data, error } = await admin.auth.admin.listUsers({ page, perPage: 100 });
    if (error) throw new Error(error.message);
    users.push(...data.users.filter((candidate) => isSyntheticQaEmail(candidate.email)));
    if (data.users.length < 100) break;
  }
  return users;
}

async function deleteSyntheticQaPrincipal({ id, email }) {
  if (!isSyntheticQaEmail(email)) {
    throw new Error(`Refusing to delete non-synthetic auth user ${id}.`);
  }
  ...
  const unexpected = (commitments ?? []).filter(
    (row) => !String(row.title).startsWith(TITLE_PREFIX),
  );
  if (unexpected.length) {
    throw new Error(`Refusing to delete synthetic user ${email} with non-QA commitments.`);
  }
  const commitmentIds = (commitments ?? []).map((row) => row.id);
  if (commitmentIds.length) {
    const { error } = await admin.from("collective_commitments").delete().in("id", commitmentIds);
    if (error) throw new Error(error.message);
  }
...
async function cleanupStaleSyntheticQaResidue() {
  const authUsers = await listSyntheticQaUsers();
  let commitmentCount = 0;
  for (const user of authUsers) {
    commitmentCount += await deleteSyntheticQaPrincipal({ id: user.id, email: user.email });
  }

  const { data: orphanProfiles, error: profileError } = await admin
    .from("profiles")
    .select("id,email")
    .like("email", `${USER_PREFIX}%`);
  if (profileError) throw new Error(profileError.message);
  const exactOrphanProfiles = (orphanProfiles ?? []).filter((row) => isSyntheticQaEmail(row.email));
  for (const profile of exactOrphanProfiles) {
    const { data: commitments, error: commitmentsError } = await admin
      .from("collective_commitments")
      .select("id,title")
    ...
    if (unexpected.length) {
      throw new Error(`Refusing to delete orphan synthetic profile ${profile.email} with non-QA commitments.`);
    }
    const ids = (commitments ?? []).map((row) => row.id);
    if (ids.length) {
      const { error } = await admin.from("collective_commitments").delete().in("id", ids);
      if (error) throw new Error(error.message);
    ...
    if (credentialError) throw new Error(credentialError.message);
    const { error } = await admin.from("profiles").delete().eq("id", profile.id).eq("email", profile.email);
    if (error) throw new Error(error.message);
  }

  const remainingAuth = await listSyntheticQaUsers();
  const { data: remainingProfiles, error: remainingProfileError } = await admin
    .from("profiles")
    .select("id,email")
    .like("email", `${USER_PREFIX}%`);
  if (remainingProfileError) throw new Error(remainingProfileError.message);
  const exactRemainingProfiles = (remainingProfiles ?? []).filter((row) => isSyntheticQaEmail(row.email));
  if (remainingAuth.length || exactRemainingProfiles.length) {
    throw new Error(
      `Synthetic QA residue remains before seeding: auth=${remainingAuth.length}, profiles=${exactRemainingProfiles.length}.`,
    );
  }
  return {
    authUsers: authUsers.length,
    orphanProfiles: exactOrphanProfiles.length,
...
async function createCommitmentDirectly({
  ...
  const typeMeta = PROPOSITION_TYPES.find((item) => item.value === propositionType);
  if (!typeMeta) throw new Error(`Unknown proposition type: ${propositionType}`);
  if (!creatorId) throw new Error("Direct commitment creation requires a creator ID.");
...
async function tableCount(table, column, value) {
  ...
  const { count, error } = await admin
    ...
    .eq(column, value);
  if (error) {
    throw new Error(`${table}: ${error.message || error.code || JSON.stringify(error)}`);
  }
  return count ?? 0;
}
...
async function writeAudit() {
  ...
  const lines = [
    ...
    ...audit.sessions.flatMap((session) => [
      ...
      `- Expected rate-limited telemetry beacons: ${session.rateLimitedFunnelEvents.length}`,
...
async function cleanup() {
  ...
  const remainingNamespaceAuth = await listSyntheticQaUsers();
  const { data: remainingNamespaceProfiles, error: remainingNamespaceProfileError } = await admin
    .from("profiles")
    .select("id,email")
    .like("email", `${USER_PREFIX}%`);
  result.remaining.syntheticQaAuthUsers = remainingNamespaceAuth.map((user) => user.email);
  result.remaining.syntheticQaProfiles = remainingNamespaceProfileError
    ? `ERROR:${remainingNamespaceProfileError.message}`
    : (remainingNamespaceProfiles ?? []).filter((row) => isSyntheticQaEmail(row.email)).length;
...
try {
  await recordCheck("remove stale synthetic QA residue", async () => {
    const removed = await cleanupStaleSyntheticQaResidue();
    return `removed ${removed.authUsers} stale auth users, ${removed.orphanProfiles} orphan profiles, and ${removed.commitments} exact QA commitments`;
  });
