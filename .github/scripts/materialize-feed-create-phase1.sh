...
printf '%s\n' "$candidate_status" > /tmp/candidate-tsc.status
python - <<'PY'
from pathlib import Path
import re
...
candidate_status = int(Path("/tmp/candidate-tsc.status").read_text().strip())
