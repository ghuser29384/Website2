...
EVIDENCE_DIR="$GITHUB_WORKSPACE/integrated-final-evidence"
...
git worktree add "$RUNNER_TEMP/main-baseline" "$EXPECTED_MAIN_SHA"
...
set +e
npm test > "$EVIDENCE_DIR/main-full-suite.log" 2>&1
MAIN_STATUS=$?
set -e
...
set +e
npm test > "$EVIDENCE_DIR/candidate-full-suite.log" 2>&1
CANDIDATE_STATUS=$?
set -e
printf '%s\n' "$MAIN_STATUS" > "$EVIDENCE_DIR/main-full-suite.status"
printf '%s\n' "$CANDIDATE_STATUS" > "$EVIDENCE_DIR/candidate-full-suite.status"
...
root = Path(os.environ["GITHUB_WORKSPACE"]) / "integrated-final-evidence"
...
(root / "normalized-full-suite-comparison.json").write_text(json.dumps(payload, indent=2) + "\n")
summary = [
...
if candidate_only or main_only or candidate_status != main_status:
    raise SystemExit("Integrated candidate does not match current main's normalized full-suite baseline.")
PY
...
npx tsc --noEmit 2>&1 | tee "$EVIDENCE_DIR/typescript.log"
...
# Refuse a race before publishing the tested merge.
git fetch origin main "$PR_BRANCH"
test "$(git rev-parse origin/main)" = "$EXPECTED_MAIN_SHA"
test "$(git rev-parse "origin/$PR_BRANCH")" = "$EXPECTED_PR_HEAD"
...
printf '%s\n' "$INTEGRATED_SHA" > "$GITHUB_WORKSPACE/integrated-sha.txt"
