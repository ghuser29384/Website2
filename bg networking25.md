...
- Include broad purpose code, broad surfaces used, authorization scope type, confidence band, factor-code counts, blocker-code counts, dependency-state label, and visible count buckets.
...
- Enforce allowed purpose code, allowed surfaces, audience/cohort scope, maximum candidates, and maximum briefs from the authorization.
- Apply rate limits.
...
- Revalidate the opportunity brief’s authorization, purpose code, profile-version, source-summary, candidate-exposure, candidate-discoverability, cohort-scope, and expiry dependencies before accepting feedback.
...
- The run stays inside the authorization’s allowed purpose codes, allowed surfaces, audience/cohort scope, maximum candidates, maximum briefs, and minimum confidence band.
...
- Candidate-side `inbound_delegate_discovery`, `inbound_delegate_purpose_codes`, `inbound_delegate_surfaces`, `allowed_cohort_ids`, and `candidate_exposure_version` authorize this requesting delegate's purpose code, audience scope, cohort, and allowed surfaces.
...
Where `anyHardBlocker01` must include prohibited-coordination blockers, purpose-mismatch blockers, authorization-scope violations, candidate-exposure violations, candidate opt-out/pause/delete states, privacy-stage violations, sparse-search or minimum-pool blockers, repeated-probe blockers, stale dependency snapshots, retention-expired or anonymized artifacts, revoked-source influence, unconfirmed uncertainty driving positive score, and safety/operator blocks. A hard blocker must suppress opportunity creation regardless of score.
...
- run budgets, brief budgets, candidate limits, allowed purpose codes, and minimum confidence bands are enforced
- broad catch-all delegate purposes are rejected
...
- candidate `inbound_delegate_purpose_codes` must be separately confirmed, nonempty for non-`off` exposure, and purpose-compatible with the requesting delegate
...
- changing candidate exposure purpose codes or other exposure settings updates `candidate_exposure_version` and marks dependent active briefs stale
...
- delegate authorizations require nonempty allowlisted purpose codes and reject broad catch-all purposes
- delegate runs and opportunity briefs snapshot the active purpose code
- candidate inbound purpose settings are enforced before scoring and before opportunity-brief display
- purpose-code removal from candidate exposure or requester authorization marks dependent active briefs stale
...
- Delegate runs require participant-controlled authorization, purpose limits, budget, expiry, and revocation.
...
- Users can create, inspect, limit, and revoke a delegate authorization with purpose codes, surfaces, scope, budgets, confidence threshold, and expiry.
...
- Prohibited-coordination, purpose-mismatch, anti-enumeration, anti-probing, uncertainty-gating, candidate-exposure, privacy-stage, revocation, and safety blockers suppress opportunity creation regardless of score.
