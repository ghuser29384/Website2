...
comment on table public.moral_trade_negative_commitment_scope_enforcement_records is
  'Append-only owner-scoped negative-commitment scope enforcement records. A row stores normalized scope input, deterministic evaluation results, blockers, and evaluation hash while enforcing that this endpoint cannot authorize lock, payment capture, reliance, abstention evidence acceptance, completion counting, public metrics, release-gate promotion, or other state changes.';

create index if not exists moral_trade_negative_commitment_scope_enforcement_owner_status_idx
  on public.moral_trade_negative_commitment_scope_enforcement_records (owner_profile_id, enforcement_status, created_at desc);

create index if not exists moral_trade_negative_commitment_scope_enforcement_transition_status_idx
  on public.moral_trade_negative_commitment_scope_enforcement_records (transition, enforcement_status, created_at desc);

create index if not exists moral_trade_negative_commitment_scope_enforcement_hash_idx
  on public.moral_trade_negative_commitment_scope_enforcement_records (evaluation_hash, created_at desc);
