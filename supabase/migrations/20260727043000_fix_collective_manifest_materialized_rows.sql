-- Forward repair for reveal-manifest validation in the collective commitment
-- activation RPC. The prior alias-only repair was applied in isolated QA, but
-- PostgreSQL still resolved the set-returning-function column incorrectly in
-- the FULL JOIN. Materialize typed JSONB rows and prefilter the commitment's
-- private signature set before exactness validation.
...
begin
  ...
  with manifest_rows as materialized (
    select jsonb_array_elements(p_manifest) as manifest_entry
  )
  select count(distinct (manifest_rows.manifest_entry->>'signatureId'))::integer
  ...
  full join manifest_rows manifest
    on (manifest.manifest_entry->>'signatureId')::uuid = signature.id
  where signature.id is null
     or manifest.manifest_entry is null
     or manifest.manifest_entry->>'revealNonce' is distinct from signature.reveal_nonce
     or manifest.manifest_entry->>'identityCommitment' is distinct from signature.identity_commitment
     or char_length(btrim(coalesce(manifest.manifest_entry->>'verifiedRealName', ''))) = 0
     or (
       manifest.manifest_entry ? 'verifiedAffiliation'
       and manifest.manifest_entry->'verifiedAffiliation' <> 'null'::jsonb
       and char_length(btrim(coalesce(manifest.manifest_entry->>'verifiedAffiliation', ''))) = 0
     )
     or encode(
          extensions.hmac(
            convert_to(
              btrim(manifest.manifest_entry->>'verifiedRealName') || E'\n' ||
              btrim(coalesce(manifest.manifest_entry->>'verifiedAffiliation', '')) || E'\n' ||
              manifest.manifest_entry->>'revealNonce',
              'UTF8'
            ),
  ...
  select
    ...
    btrim(manifest.manifest_entry->>'verifiedRealName'),
    nullif(btrim(coalesce(manifest.manifest_entry->>'verifiedAffiliation', '')), ''),
    signature.signed_at,
    revealed_at_value,
