-- Tighten integrity symmetry, late-cure semantics, and current-final payment adjudication.

alter table public.trade_evidence_decisions
  drop constraint if exists trade_evidence_decisions_contradiction_integrity_consistency_check;
alter table public.trade_evidence_decisions
  add constraint trade_evidence_decisions_contradiction_integrity_consistency_check check (
    (
      contradiction_status = 'materially_reckless'
      and integrity_finding = 'reckless_misleading'
    )
    or (
      contradiction_status = 'deliberate'
      and integrity_finding = 'deliberate_fabrication'
    )
    or (
      contradiction_status not in ('materially_reckless', 'deliberate')
      and integrity_finding not in ('reckless_misleading', 'deliberate_fabrication')
    )
  );
...
begin
  ...
  elsif p_integrity_finding = 'reckless_misleading'
     and p_contradiction_status <> 'materially_reckless' then
    raise exception 'Recklessly misleading evidence requires a materially reckless contradiction finding.';
  elsif p_integrity_finding = 'deliberate_fabrication'
     and p_contradiction_status <> 'deliberate' then
    raise exception 'Deliberate fabrication requires a deliberate contradiction finding.';
  end if;

  if p_finality_reason = 'late_cure'
     and (p_review_id is null or review_row.outcome <> 'graded') then
    raise exception 'Late cure requires a final graded review.';
  end if;
