...
as $$
  select array[
    ...
    'appendOnlyRegistryRecord','protectedTagOrEquivalent','amendmentLog',
    'deviationLog'
  ]::text[];
$$;

create or replace function public.impact_study_payload_has_required_bindings(
  p_payload jsonb
)
returns boolean
language sql
...
create table public.impact_study_protocol_bindings (
  ...
  constraint impact_study_protocol_bindings_hash_check
    check (public.impact_study_is_sha256(payload_sha256)),
  constraint impact_study_protocol_bindings_commit_check
    check (merged_commit_sha ~ '^[0-9a-f]{40}$'),
  constraint impact_study_protocol_bindings_status_check
    check (binding_status = 'accepted_internal_design_contract'),
  constraint impact_study_protocol_bindings_no_execution_check
...
create table public.impact_study_template_bindings (
  ...
  constraint impact_study_template_bindings_mechanism_check
    check (
      mechanism_family in (
  ...
  constraint impact_study_template_bindings_hash_check
    check (public.impact_study_is_sha256(payload_sha256)),
  constraint impact_study_template_bindings_status_check
    check (template_status = 'bound_nonexecuting'),
  constraint impact_study_template_bindings_no_execution_check
    check (execution_authorized = false)
);

insert into public.impact_study_protocol_bindings (
  protocol_key,payload_sha256,repository,merged_commit_sha,
  accepted_review_id,binding_status,execution_authorized
) values (
...
insert into public.impact_study_template_bindings (
  mechanism_family,template_key,payload_sha256,template_status,
...
create table public.impact_study_instances (
  ...
  constraint impact_study_instances_version_check check (study_version > 0),
  constraint impact_study_instances_mechanism_check
    check (
      mechanism_family in (
        'trade','co_fund','threshold_funding','donation_upgrade',
        'threshold_sign_on','donation_redirect'
  ...
  constraint impact_study_instances_payload_object_check
    check (jsonb_typeof(study_instance_payload) = 'object'),
  constraint impact_study_instances_hashes_check
    check (
      public.impact_study_is_sha256(protocol_payload_hash)
      and public.impact_study_is_sha256(template_payload_hash)
      and public.impact_study_is_sha256(study_instance_payload_hash)
      and public.impact_study_is_sha256(eligible_population_snapshot_hash)
      and public.impact_study_is_sha256(assignment_code_hash)
      and public.impact_study_is_sha256(analysis_code_hash)
      and public.impact_study_is_sha256(seed_commitment)
    ),
  constraint impact_study_instances_environment_check check (environment = 'qa'),
  constraint impact_study_instances_subject_mode_check check (subject_mode = 'synthetic_only'),
  constraint impact_study_instances_no_execution_check check (execution_authorized = false),
  constraint impact_study_instances_no_real_users_check check (real_user_assignment_allowed = false),
  constraint impact_study_instances_status_check check (registry_status = 'registered_nonexecuting'),
  constraint impact_study_instances_registry_uri_check check (append_only_registry_uri ~ '^qa-registry://'),
  constraint impact_study_instances_protocol_fk
    foreign key (protocol_key, protocol_payload_hash)
    references public.impact_study_protocol_bindings(protocol_key, payload_sha256),
  constraint impact_study_instances_template_fk
...
create table public.impact_study_registry_events (
  ...
  constraint impact_study_registry_events_type_check
    check (
      event_type in (
  ...
  constraint impact_study_registry_events_payload_check
    check (jsonb_typeof(event_payload) = 'object'),
  constraint impact_study_registry_events_hash_check
    check (public.impact_study_is_sha256(event_payload_sha256)),
  constraint impact_study_registry_events_no_execution_check
    check (execution_effect = 'none'),
  constraint impact_study_registry_events_synthetic_check
...
create table public.impact_study_synthetic_assignments (
  ...
  constraint impact_study_assignments_subject_check
    check (public.impact_study_is_synthetic_key(synthetic_subject_key)),
  constraint impact_study_assignments_cluster_check
    check (public.impact_study_is_synthetic_key(synthetic_cluster_key)),
  constraint impact_study_assignments_probability_check
    check (assignment_probability > 0 and assignment_probability <= 1),
  constraint impact_study_assignments_payload_check
    check (jsonb_typeof(assignment_payload) = 'object'),
  constraint impact_study_assignments_hash_check
    check (public.impact_study_is_sha256(assignment_payload_sha256)),
  constraint impact_study_assignments_no_real_user_check
    check (is_real_user = false)
);
...
create table public.impact_study_synthetic_exposures (
  ...
  exposure_payload jsonb not null default '{}'::jsonb,
  exposure_payload_sha256 text not null,
  observed_at timestamptz not null default statement_timestamp(),
  unique (assignment_id,exposure_event_key),
  constraint impact_study_exposures_payload_check
    check (jsonb_typeof(exposure_payload) = 'object'),
  constraint impact_study_exposures_hash_check
...
create table public.impact_study_synthetic_outcomes (
  ...
  constraint impact_study_outcomes_subject_check
    check (public.impact_study_is_synthetic_key(synthetic_subject_key)),
  constraint impact_study_outcomes_resolution_check
    check (resolution_status in ('reviewed','unresolved','missing','rejected')),
  constraint impact_study_outcomes_reviewed_check
    check (
      resolution_status <> 'reviewed'
      or (numeric_value is not null and cardinality(evidence_refs) > 0)
    ),
  constraint impact_study_outcomes_payload_check
    check (jsonb_typeof(outcome_payload) = 'object'),
  constraint impact_study_outcomes_hash_check
    check (public.impact_study_is_sha256(outcome_payload_sha256)),
  constraint impact_study_outcomes_no_causal_claim_check
    check (
      causal_claim_authorized = false
      and additionality_claim_authorized = false
      and participant_credit_authorized = false
    )
);
...
create table public.impact_study_safety_events (
  ...
  safety_outcome_key text not null,
  safety_status text not null,
  blocking_veto boolean not null default true,
  safety_payload jsonb not null default '{}'::jsonb,
  safety_payload_sha256 text not null,
  recorded_at timestamptz not null default statement_timestamp(),
  constraint impact_study_safety_subject_check
    check (
      synthetic_subject_key is null
      or public.impact_study_is_synthetic_key(synthetic_subject_key)
    ),
  constraint impact_study_safety_outcome_check
    check (
      safety_outcome_key in (
        'harmful_offer_or_threat','baseline_manufacture_or_worsening',
        'harm_shifted_to_nonparticipants',
        'coercion_harassment_identity_exposure_or_retaliation',
        'concentration_or_exclusion_effect','off_platform_substitution',
        'duplicate_or_overlapping_resource_claim'
      )
    ),
  constraint impact_study_safety_status_check
    check (safety_status in ('observed','unresolved','cleared')),
  constraint impact_study_safety_veto_check check (blocking_veto = true),
  constraint impact_study_safety_payload_check
    check (jsonb_typeof(safety_payload) = 'object'),
  constraint impact_study_safety_hash_check
...
create table public.impact_study_calibration_manifests (
  ...
  constraint impact_study_calibration_count_check check (observation_count >= 0),
  constraint impact_study_calibration_scope_check check (source_scope = 'synthetic_qa_only'),
  constraint impact_study_calibration_holdout_check check (holdout_status = 'not_applicable_synthetic'),
  constraint impact_study_calibration_ineligible_check
    check (
      eligible_for_empirical_calibration = false
      and eligible_for_model_activation = false
    ),
  constraint impact_study_calibration_payload_check
    check (jsonb_typeof(manifest_payload) = 'object'),
  constraint impact_study_calibration_hash_check
...
declare
  expected_hash text;
begin
  if not public.impact_study_payload_has_required_bindings(
    new.study_instance_payload
  ) then
    raise exception 'Study instance is missing one or more required protocol bindings'
      using errcode = '23514';
  end if;

  if public.impact_study_payload_contains_real_identifiers(
    new.study_instance_payload
  ) then
    raise exception 'QA study payload contains a prohibited real-person identifier field'
      using errcode = '23514';
  end if;

  if new.study_instance_payload ->> 'studyKey' <> new.study_key
    or (new.study_instance_payload ->> 'studyVersion')::integer <> new.study_version
    or new.study_instance_payload ->> 'mechanismFamily' <> new.mechanism_family
    or new.study_instance_payload ->> 'studyVariant' <> new.study_variant
    or new.study_instance_payload ->> 'protocolPayloadHash'
      <> new.protocol_payload_hash
    or new.study_instance_payload ->> 'templatePayloadHash'
      <> new.template_payload_hash
    or new.study_instance_payload ->> 'studyInstancePayloadHash'
      <> new.study_instance_payload_hash
    or new.study_instance_payload ->> 'eligiblePopulationSnapshotHash'
      <> new.eligible_population_snapshot_hash
    or new.study_instance_payload ->> 'assignmentCodeHash'
      <> new.assignment_code_hash
    or new.study_instance_payload ->> 'analysisCodeHash'
      <> new.analysis_code_hash
    or new.study_instance_payload ->> 'seedCommitment'
      <> new.seed_commitment
    or new.study_instance_payload ->> 'appendOnlyRegistryRecord'
      <> new.append_only_registry_uri
    or new.study_instance_payload ->> 'protectedTagOrEquivalent'
      <> new.protected_record_ref
  then
    raise exception 'Study instance columns do not match the immutable payload'
      using errcode = '23514';
  end if;

  if new.study_instance_payload ->> 'instrumentationEnvironment' <> 'qa'
    or new.study_instance_payload ->> 'subjectMode' <> 'synthetic_only'
    or new.study_instance_payload -> 'executionAuthorized'
      is distinct from 'false'::jsonb
    or new.study_instance_payload -> 'realUserAssignmentAllowed'
      is distinct from 'false'::jsonb
  then
    raise exception 'Study instrumentation must remain QA-only, synthetic-only, and non-executing'
      using errcode = '23514';
  end if;

  if not public.impact_study_is_sha256(
    new.study_instance_payload ->> 'assignmentCodeHash'
  ) then
    raise exception 'Assignment code hash is missing or malformed'
      using errcode = '23514';
  end if;
  ...
  if expected_hash <> new.study_instance_payload_hash then
    raise exception 'Study-instance canonical database payload hash mismatch'
      using errcode = '23514';
  end if;
...
exception
  when invalid_text_representation then
    raise exception 'Study version must be a positive integer'
      using errcode = '23514';
end;
$$;
...
begin
  ...
  if parent_id is null or not exists (
    ...
    raise exception 'Child instrumentation requires a registered non-executing QA study instance'
      using errcode = '23514';
  end if;
  ...
  if tg_table_name = 'impact_study_synthetic_outcomes'
    and not exists (
      select 1
      from public.impact_study_synthetic_assignments assignment
      where assignment.study_instance_id = new.study_instance_id
...
begin
  insert into public.impact_study_instances (
    ...
    protocol_key,protocol_payload_hash,template_key,template_payload_hash,
    study_instance_payload,study_instance_payload_hash,
    eligible_population_snapshot_hash,assignment_code_hash,
    analysis_code_hash,seed_commitment,append_only_registry_uri,
  ...
  insert into public.impact_study_registry_events (
    ...
    new_id,'study_registered',registration_event,
...
begin
  insert into public.impact_study_registry_events (
    ...
    p_study_instance_id,p_event_type,p_event_payload,
...
create or replace function public.record_qa_synthetic_assignment(
  ...
  p_assignment_payload jsonb default '{}'::jsonb
)
returns uuid
language plpgsql
security definer
...
begin
  insert into public.impact_study_synthetic_assignments (
    study_instance_id,synthetic_subject_key,synthetic_cluster_key,arm_key,
    assignment_probability,planned_exposure_cell,assignment_payload,
  ...
  perform public.append_qa_impact_study_event(
    p_study_instance_id,
...
create or replace function public.record_qa_synthetic_exposure(
  ...
  p_exposure_payload jsonb default '{}'::jsonb
)
returns uuid
language plpgsql
security definer
...
begin
  ...
  insert into public.impact_study_synthetic_exposures (
    ...
    contamination_detected,spillover_detected,exposure_payload,
    exposure_payload_sha256
  ) values (
    p_assignment_id,p_exposure_event_key,p_observed_exposure_cell,
    p_contamination_detected,p_spillover_detected,p_exposure_payload,
...
create or replace function public.record_qa_synthetic_outcome(
  ...
  p_outcome_payload jsonb default '{}'::jsonb
)
returns uuid
language plpgsql
security definer
...
begin
  insert into public.impact_study_synthetic_outcomes (
    study_instance_id,synthetic_subject_key,outcome_key,native_unit,
    numeric_value,resolution_status,evidence_refs,outcome_payload,
    outcome_payload_sha256
  ) values (
    p_study_instance_id,p_synthetic_subject_key,p_outcome_key,p_native_unit,
    p_numeric_value,p_resolution_status,coalesce(p_evidence_refs,'{}'::text[]),
    p_outcome_payload,public.impact_study_jsonb_sha256(p_outcome_payload)
  ) returning id into new_id;
...
create or replace function public.record_qa_impact_safety_event(
  ...
  p_safety_payload jsonb default '{}'::jsonb
)
returns uuid
language plpgsql
security definer
...
begin
  insert into public.impact_study_safety_events (
    study_instance_id,synthetic_subject_key,safety_outcome_key,
    safety_status,safety_payload,safety_payload_sha256
  ) values (
    p_study_instance_id,p_synthetic_subject_key,p_safety_outcome_key,
    p_safety_status,p_safety_payload,
...
begin
  insert into public.impact_study_calibration_manifests (
    study_instance_id,dataset_key,observation_count,manifest_payload,
    manifest_payload_sha256
  ) values (
    p_study_instance_id,p_dataset_key,p_observation_count,p_manifest_payload,
...
revoke all on table public.impact_study_protocol_bindings from public,anon,authenticated,service_role;
revoke all on table public.impact_study_template_bindings from public,anon,authenticated,service_role;
revoke all on table public.impact_study_instances from public,anon,authenticated,service_role;
revoke all on table public.impact_study_registry_events from public,anon,authenticated,service_role;
revoke all on table public.impact_study_synthetic_assignments from public,anon,authenticated,service_role;
revoke all on table public.impact_study_synthetic_exposures from public,anon,authenticated,service_role;
revoke all on table public.impact_study_synthetic_outcomes from public,anon,authenticated,service_role;
revoke all on table public.impact_study_safety_events from public,anon,authenticated,service_role;
revoke all on table public.impact_study_calibration_manifests from public,anon,authenticated,service_role;

grant select on table public.impact_study_protocol_bindings to service_role;
grant select on table public.impact_study_template_bindings to service_role;
grant select on table public.impact_study_instances to service_role;
grant select on table public.impact_study_registry_events to service_role;
grant select on table public.impact_study_synthetic_assignments to service_role;
grant select on table public.impact_study_synthetic_exposures to service_role;
grant select on table public.impact_study_synthetic_outcomes to service_role;
grant select on table public.impact_study_safety_events to service_role;
grant select on table public.impact_study_calibration_manifests to service_role;

revoke execute on function public.register_qa_impact_study_instance(jsonb) from public,anon,authenticated;
revoke execute on function public.append_qa_impact_study_event(uuid,text,jsonb) from public,anon,authenticated;
revoke execute on function public.record_qa_synthetic_assignment(uuid,text,text,text,numeric,text,jsonb) from public,anon,authenticated;
revoke execute on function public.record_qa_synthetic_exposure(uuid,text,text,boolean,boolean,jsonb) from public,anon,authenticated;
revoke execute on function public.record_qa_synthetic_outcome(uuid,text,text,text,numeric,text,text[],jsonb) from public,anon,authenticated;
revoke execute on function public.record_qa_impact_safety_event(uuid,text,text,text,jsonb) from public,anon,authenticated;
revoke execute on function public.register_qa_synthetic_calibration_manifest(uuid,text,integer,jsonb) from public,anon,authenticated;

grant execute on function public.register_qa_impact_study_instance(jsonb) to service_role;
grant execute on function public.append_qa_impact_study_event(uuid,text,jsonb) to service_role;
grant execute on function public.record_qa_synthetic_assignment(uuid,text,text,text,numeric,text,jsonb) to service_role;
grant execute on function public.record_qa_synthetic_exposure(uuid,text,text,boolean,boolean,jsonb) to service_role;
grant execute on function public.record_qa_synthetic_outcome(uuid,text,text,text,numeric,text,text[],jsonb) to service_role;
grant execute on function public.record_qa_impact_safety_event(uuid,text,text,text,jsonb) to service_role;
grant execute on function public.register_qa_synthetic_calibration_manifest(uuid,text,integer,jsonb) to service_role;
