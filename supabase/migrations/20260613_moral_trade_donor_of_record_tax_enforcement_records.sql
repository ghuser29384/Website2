...
comment on table public.moral_trade_donor_of_record_tax_enforcement_records is
  'Append-only owner-scoped donor-of-record tax enforcement records. A row stores normalized donor/tax input, deterministic evaluation results, blockers, and evaluation hash while enforcing that this endpoint cannot authorize lock, payment authorization, payment capture, receipt issuance, public metrics, release-gate promotion, or other state changes.';

create index if not exists moral_trade_donor_tax_enforcement_owner_status_idx
  on public.moral_trade_donor_of_record_tax_enforcement_records (owner_profile_id, enforcement_status, created_at desc);

create index if not exists moral_trade_donor_tax_enforcement_transition_status_idx
  on public.moral_trade_donor_of_record_tax_enforcement_records (transition, enforcement_status, created_at desc);

create index if not exists moral_trade_donor_tax_enforcement_hash_idx
  on public.moral_trade_donor_of_record_tax_enforcement_records (evaluation_hash, created_at desc);
