# Coalition-Routed Escrowed Conditional Matching (CRECM v1.5)
...
Build **Coalition-Routed Escrowed Conditional Matching v1.5** in `moraltrade.org`.

- **Abbreviation:** CRECM
- **User-facing label:** Common Ground Budget
- **Technical label:** CRECM v1.5
...
const actualAllocCents =
  budgetEligible && ["strong", "weak"].includes(stance)
    ? min(
...
Reroute and carry-forward must never silently expand the user's exposure, change the recipient class, change the moral bucket, change the accepted counterparty buckets, or operate under a materially different rulebook. If those terms differ, the fallback becomes `refund` / `release_hold` until the user gives fresh consent.
...
const validatedCounterBucketIds = intersection(
...
```env
MPGF_MECHANISM_VERSION=crecm_v1_5
```
...
```text
Coalition-Routed Escrowed Conditional Matching v1.5
```
...
> **CRECM v1.5: Common Ground Budget + cross-view conditional clearing + supervised escrow/custody + hard review gates + separated actual/counted/match-eligible accounting + active-budget consent gating + enforced identity thresholds + sponsor-pool precommitment + base match + stance-consistent capped diversity-aware post-clear bonus + intent-capped allocation + distinct-bucket counterparty validation + explicit failure handling.**
