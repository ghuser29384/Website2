...
import { INSTITUTIONAL_RESOURCE_TYPES, ORGANIZATION_INSTITUTIONAL_NAV } from "@/lib/institutional-trade";
...
export default async function InstitutionalDealPage({ params, searchParams }: PageProps) {
  ...
  const organizationParty = organizationParties.find((party) => party.organization_id === organizationId) ?? null;
  ...
  return <div className={styles.shell}>
    ...
    <header className={styles.hero}><div className={styles.heroInner}>
      <div className={styles.heroCopy}>
        ...
        <nav aria-label="Organization deal workspace" className={styles.subnav}>{ORGANIZATION_INSTITUTIONAL_NAV.map((item) => <a href={`#${item.toLowerCase().replaceAll(" ", "-")}`} key={item}>{item}</a>)}</nav>
      </div>
    ...
    <main className={styles.main}>
      ...
      <section className={styles.section}>
        ...
        <InstitutionalDisclosure title="Record an exact-scope baseline"><form action={runInstitutionalAction} className={styles.formGrid}>
          {formBase("create_baseline", returnTo, dealId, organizationId)}
        ...
        <InstitutionalDisclosure title="Invite an organization or named person"><form action={runInstitutionalAction} className={styles.formGrid}>
          {formBase("create_deal_party", returnTo, dealId, organizationId)}
      ...
      <section className={styles.section} id="proposals">
        ...
        {data.proposals.length ? <div className={styles.grid}>{data.proposals.map((proposal) => <article className={styles.card} key={proposal.id}>
          ...
          {proposal.status === "proposed" && !selectedProposal ? <form action={runInstitutionalAction} className={styles.form}>
            {formBase("select_proposal", `${returnTo}#proposals`, dealId, organizationId)}
        ...
        <InstitutionalDisclosure title="Create a proposal version"><form action={runInstitutionalAction} className={styles.formGrid}>
          {formBase("create_proposal", `${returnTo}#proposals`, dealId, organizationId)}
      ...
      <section className={styles.section}>
        ...
        {selectedProposal && selectedObligations.filter((obligation) => obligation.individual_consent_required).map((obligation) => {
          ...
          return existing ? null : <form action={runInstitutionalAction} className={styles.inlineActions} key={obligation.id}>
            {formBase("request_individual_consent", `${returnTo}#proposals`, dealId, organizationId)}<input name="obligationId" type="hidden" value={obligation.id} />
        ...
        {selectedProposal ? <InstitutionalDisclosure title="Sign selected exact terms"><form action={runInstitutionalAction} className={styles.formGrid}>
          {formBase("sign_deal", returnTo, dealId, organizationId)}<input name="expectedTermsHash" type="hidden" value={selectedProposal.terms_hash} />
      ...
      <section className={styles.section}>
        ...
        {selectedProposal ? <InstitutionalDisclosure title="Create a typed obligation"><form action={runInstitutionalAction} className={styles.formGrid}>
          {formBase("create_obligation", returnTo, dealId, organizationId)}<input name="proposalVersionId" type="hidden" value={selectedProposal.id} />
      ...
      {data.pool ? <section className={styles.section} id="pool">
        ...
        <div className={styles.metricGrid}><InstitutionalMetric label="Threshold" value={formatInstitutionalMoney(data.pool.threshold_amount_cents, data.pool.currency)} note={`${data.pool.minimum_contributors} minimum contributors`} /><InstitutionalMetric label="Committed" value={formatInstitutionalMoney(committedCents, data.pool.currency)} note={`${committedContributions.length} contributors`} /><InstitutionalMetric label="Governance votes" value={data.votes.length} note={formatInstitutionalLabel(data.pool.governance_rule)} /><InstitutionalMetric label="Pool status" value={formatInstitutionalLabel(data.pool.status)} note={`Terms ${String(data.pool.terms_hash).slice(0, 12)}…`} /></div>
