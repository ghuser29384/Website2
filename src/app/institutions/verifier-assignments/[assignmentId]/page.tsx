...
export default async function VerifierAssignmentPage({ params, searchParams }: PageProps) {
  ...
  return <div className={styles.shell}><SiteTopbar brandHref="/" links={getPrimaryNavLinks(true)} authLink={actions.authLink} primaryAction={actions.primaryAction} showLogout />
    ...
    <main className={styles.main}>{message ? <p className={message.tone === "error" ? styles.errorNotice : styles.successNotice}>{message.text}</p> : null}<section className={styles.section}><InstitutionalSectionHeader title="Assignment decision" description="Accept only when you can independently review the stated scope. Declining grants no confidential access." />
      <article className={styles.panel}><div className={styles.cardHeader}><h3>Assignment status</h3><InstitutionalStatus tone={institutionalStatusTone(assignment.status)}>{formatInstitutionalLabel(assignment.status)}</InstitutionalStatus></div><InstitutionalKeyValue entries={[["Scope", assignment.scope],["Assigned", new Date(assignment.created_at).toLocaleString()],["Accepted", assignment.accepted_at ? new Date(assignment.accepted_at).toLocaleString() : "Not accepted"],["Conflict declaration", assignment.conflict_declaration || "Not recorded"]]} /></article>
      {assignment.status === "invited" ? <form action={runInstitutionalAction} className={styles.formGrid}><input name="actionType" type="hidden" value="decide_verifier_assignment" /><input name="returnTo" type="hidden" value={returnTo} /><input name="assignmentId" type="hidden" value={assignment.id} /><label>Decision<select name="decision" defaultValue="accepted"><option value="accepted">Accept assignment</option><option value="declined">Decline assignment</option></select></label><label className={styles.fullSpan}>Conflict declaration<textarea name="conflictDeclaration" required /></label><div className={`${styles.formActions} ${styles.fullSpan}`}><button className={styles.primaryButton} type="submit">Record my decision</button></div></form> : <p className={styles.callout}>Assignment is {formatInstitutionalLabel(assignment.status)}. Confidential access exists only while an accepted assignment remains active.</p>}
      <div className={styles.inlineActions}>{deal ? <Link className={styles.secondaryButton} href={`/institutions/${deal.lead_organization_id || ""}/deals/${deal.id}`}>Open deal room after acceptance</Link> : null}<Link className={styles.secondaryButton} href="/institutions">Institutional directory</Link></div>
