...
import { INSTITUTIONAL_RISK_CATEGORIES, INSTITUTIONAL_RISK_SEVERITIES } from "@/lib/institutional-trade";
            ...
            <section className={styles.section}>
              ...
              <div className={styles.twoColumn}>
                ...
                <div><h3>Open disputes</h3>{desk.disputes.length ? <div className={styles.timeline}>{desk.disputes.map((dispute) => {
                  ...
                  return <article className={styles.timelineItem} key={dispute.id}><time><InstitutionalDate value={dispute.updated_at} /></time><div><strong>{text(deal?.title, id(dispute.deal_id))} · {formatInstitutionalLabel(dispute.stage)}</strong><p>Opened by {mapName(organizationById, party?.organization_id)} · {text(dispute.summary)}</p><Link href={`/institutions/${id(deal?.lead_organization_id)}/deals/${id(dispute.deal_id)}#disputes`}>Open dispute record</Link></div></article>;
