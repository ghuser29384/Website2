...
export async function POST(request: Request) {
  ...
  const briefId = stringField(body.briefId ?? body.brief_id);
  const reason =
  ...
  const requestedOutcome = normalizeBackgroundOpportunityFeedbackOutcome(
    stringField(body.outcome ?? body.decision),
  );
  const outcome =
    requestedOutcome ??
  ...
  if (!briefId) {
    return privateJson({ error: "Opportunity brief id is required." }, 400);
  }
  ...
  if (!isBackgroundOpportunityFeedbackPairAllowed({ outcome, reasonCode: reason })) {
    return privateJson({ error: "Choose a supported opportunity feedback reason." }, 400);
  }
