...
export async function GET(_request: Request, { params }: { params: Promise<{ roundId: string }> }) {
  ...
  const fallbackResult = getMpgfPublicGoodsMatchPreviewApi(roundId);
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          warnings: [
            error instanceof Error
              ? `Could not load persisted MPGF match preview state: ${error.message}`
              : "Could not load persisted MPGF match preview state.",
          ],
        },
        { headers: MPGF_PUBLIC_GOODS_API_HEADERS },
      );
    ...
    return NextResponse.json(
      {
        ok: false,
        error: error instanceof Error ? error.message : "Could not load MPGF public-goods match preview.",
      },
