...
function InteractiveField({ className, caption }: Pick<GainFieldProps, "className" | "caption">) {
  ...
  const activateCandidate = (o: Option) => {
    if (pinnedRef.current) { releasePoint(); return; }
    select(o);
  };
  ...
  const down = (e: ReactPointerEvent<SVGSVGElement>) => {
    ...
    e.preventDefault();
    if (pinnedRef.current) { releasePoint(); return; }
    custom(fromClient(e.clientX, e.clientY), true);
  };
  ...
  const selectedAction = pinned ? "It is fixed. Click again or press Escape to resume pointer tracking." : "It follows pointer movement. Click the field to fix it.";
  ...
  return <figure className={[styles.field, className].filter(Boolean).join(" ")}>
    ...
    <div className={styles.stage}>
      <svg aria-labelledby={`interactive-gain-title-${id} interactive-gain-desc-${id}`} className={styles.canvas} onPointerDown={down} onPointerMove={move} ref={svg} role="img" style={{ cursor: "crosshair" }} viewBox="0 0 720 520">
        ...
        <desc id={`interactive-gain-desc-${id}`}>The proposal follows pointer movement. Click once anywhere in the field to fix it at that point; click again to release it and resume tracking. It qualifies only above and to the right of the no-deal default.</desc>
        ...
        <g aria-label={`Proposed agreement. ${comparison(gain.victoria)} for Victoria; ${comparison(gain.paul)} for Paul. ${selectedAction}`} aria-pressed={pinned} className={[styles.selected, status.mutual ? "" : styles.selectedBlocked].filter(Boolean).join(" ")} onClick={togglePoint} onKeyDown={key} onPointerDown={(e) => e.stopPropagation()} role="button" style={{ cursor: "pointer" }} tabIndex={0} transform={`translate(${point.x} ${point.y})`}><title>{pinned ? "Fixed proposal; click again to resume tracking" : "Click to fix proposal"}</title><circle className={styles.selectedPulse} r="30" /><circle className={styles.selectedOuter} r="19" /><circle className={styles.selectedInner} r="6" /></g>
