...
test("api contract profile publishes core routes, schemas, privacy classes, and fallbacks", () => {
  ...
  assert.ok(
    profile.routes.some(
      (route) =>
        route.key === "moral_trade_marketplace_state_event_contract" &&
        route.path === "/api/moral-trade/marketplace-state-events/contract" &&
        route.responseSchema === "marketplace_state_event_contract_response",
    ),
  );
  assert.ok(
    profile.routes.some(
      (route) =>
        route.key === "moral_trade_provider_source_authentication_contract" &&
        route.path === "/api/moral-trade/provider-source-authentication/contract" &&
        route.responseSchema === "provider_source_authentication_contract_response",
    ),
  );
  assert.ok(
    profile.routes.some(
  ...
  assert.ok(
    profile.routes.some(
      (route) =>
        route.key === "moral_trade_baseline_integrity_enforce" &&
        route.path === "/api/moral-trade/baseline-integrity/enforce" &&
        route.auth === "authenticated" &&
        route.cacheControl === "private_no_store" &&
        route.rateLimitSurface === "baseline_integrity_enforce" &&
        route.requestSchema === "baseline_integrity_enforce_request" &&
        route.responseSchema === "baseline_integrity_enforce_response",
    ),
  );
  assert.ok(
    profile.routes.some(
  ...
  assert.ok(
    profile.routes.some(
      (route) =>
        route.key === "moral_trade_recipient_acceptance_contract" &&
        route.path === "/api/moral-trade/recipient-acceptance/contract" &&
        route.responseSchema === "recipient_acceptance_contract_response",
    ),
  );
  assert.ok(
    profile.routes.some(
      (route) =>
        route.key === "moral_trade_ai_preference_elicitation_contract" &&
        route.path === "/api/moral-trade/ai-preference-elicitation/contract" &&
        route.responseSchema === "ai_preference_elicitation_contract_response",
    ),
  );
  assert.ok(
    profile.routes.some(
      (route) =>
        route.key === "moral_trade_post_clear_audit_contract" &&
        route.path === "/api/moral-trade/post-clear-audit/contract" &&
        route.responseSchema === "post_clear_audit_contract_response",
    ),
  );
  assert.ok(
    profile.routes.some(
      (route) =>
        route.key === "moral_trade_non_public_goods_subsidy_contract" &&
        route.path === "/api/moral-trade/non-public-goods-subsidies/contract" &&
        route.responseSchema === "non_public_goods_subsidy_contract_response",
    ),
  );
  assert.ok(
    profile.routes.some(
      (route) =>
        route.key === "moral_trade_direct_pair_clearing_contract" &&
        route.path === "/api/moral-trade/direct-pair-clearing/contract" &&
        route.responseSchema === "direct_pair_clearing_contract_response",
    ),
  );
  assert.ok(
    profile.routes.some(
  ...
  assert.ok(
    profile.routes.some(
      (route) =>
        route.key === "moral_trade_resource_compatibility_contract" &&
        route.path === "/api/moral-trade/resource-compatibility/contract" &&
        route.responseSchema === "resource_compatibility_contract_response",
    ),
  );
  assert.ok(
    profile.routes.some(
      (route) =>
        route.key === "moral_trade_net_offset_accounting_contract" &&
        route.path === "/api/moral-trade/net-offset-accounting/contract" &&
        route.responseSchema === "net_offset_accounting_contract_response",
    ),
  );
  assert.ok(
    profile.routes.some(
      (route) =>
        route.key === "moral_trade_offer_validity_contract" &&
        route.path === "/api/moral-trade/offer-validity/contract" &&
        route.responseSchema === "offer_validity_contract_response",
    ),
  );
  assert.ok(
    profile.routes.some(
      (route) =>
        route.key === "moral_trade_private_exchange_rate_contract" &&
        route.path === "/api/moral-trade/private-exchange-rate/contract" &&
        route.responseSchema === "private_exchange_rate_contract_response",
    ),
  );
  assert.ok(
    profile.routes.some(
      (route) =>
        route.key === "moral_trade_noncompensable_blocker_contract" &&
        route.path === "/api/moral-trade/noncompensable-blockers/contract" &&
        route.responseSchema === "noncompensable_blocker_contract_response",
    ),
  );
  assert.ok(
    profile.routes.some(
  ...
  assert.ok(
    profile.routes.some(
      (route) =>
        route.key === "moral_trade_pilot_evidence_contract" &&
        route.path === "/api/moral-trade/pilot-evidence/contract" &&
        route.responseSchema === "pilot_evidence_contract_response",
    ),
  );
  assert.ok(
    profile.routes.some(
  ...
  assert.ok(
    profile.routes.some(
      (route) =>
        route.key === "moral_trade_payment_authorization_contract" &&
        route.path === "/api/moral-trade/payment-authorizations/contract" &&
        route.responseSchema === "payment_authorization_contract_response",
    ),
  );
  ...
  assert.ok(profile.schemaDefinitions.some((schema) => schema.key === "recipient_acceptance_contract_response"));
  assert.ok(profile.schemaDefinitions.some((schema) => schema.key === "ai_preference_elicitation_contract_response"));
  ...
  assert.ok(profile.schemaDefinitions.some((schema) => schema.key === "resource_compatibility_contract_response"));
  ...
  assert.ok(profile.schemaDefinitions.some((schema) => schema.key === "pilot_evidence_contract_response"));
  ...
  assert.ok(
    profile.schemaDefinitions
      .find((schema) => schema.key === "moral_trade_aggregate_health_response")
      ?.fields.some((field) => field.key === "pilotEvidenceValidation"),
  );
  assert.ok(
    profile.schemaDefinitions
      .find((schema) => schema.key === "moral_trade_aggregate_health_response")
  ...
  assert.ok(
    profile.schemaDefinitions
      .find((schema) => schema.key === "moral_trade_aggregate_health_response")
      ?.fields.some((field) => field.key === "recipientAcceptanceValidation"),
  );
  assert.ok(
    profile.schemaDefinitions
      .find((schema) => schema.key === "moral_trade_aggregate_health_response")
      ?.fields.some((field) => field.key === "aiPreferenceElicitationValidation"),
  );
  assert.ok(
    profile.schemaDefinitions
      .find((schema) => schema.key === "moral_trade_aggregate_health_response")
