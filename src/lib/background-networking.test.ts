...
test("deterministic background matches do not return raw overlapping private terms", () => {
  const evaluation = evaluateDeterministicMatch({
    ...
    } as unknown as DeterministicMatchInput["counterparty"],
    counterpartySignals: {
      ...baseSignals,
...
test("deterministic synthesis counts only active unexpired source permissions", () => {
  ...
  const synthesis = buildDeterministicSynthesis({
    ...
    } as unknown as SynthesisInput["profile"],
    profileSources: [],
  });
...
test("deterministic synthesis reads only approved active profile signals", () => {
  const signal = (
    ...
    ({
      ...
      confirmation_actor_profile_id: "profile-1",
      confirmation_kind: "explicit_participant_confirmation",
      confirmation_policy_version: "background-source-summary-tag-confirmation-v1",
      confirmed_at: "2026-06-14T00:00:00.000Z",
      expires_at: "2099-01-01T00:00:00.000Z",
      lineage_status: "active",
      signal_fingerprint: "sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
      sensitivity: "broad",
...
test("deterministic synthesis ignores approved source-summary proposals until tag confirmation", () => {
  const signal = (
    overrides: Partial<NonNullable<SynthesisInput["profileSignals"]>[number]>,
  ): NonNullable<SynthesisInput["profileSignals"]>[number] =>
    ({
      ...
      sensitivity: "broad",
      signal_key: "capability_tag",
      signal_value: "unconfirmedsourceproposal",
      source: "approved_source_summary",
      status: "active",
      ...overrides,
    }) as NonNullable<SynthesisInput["profileSignals"]>[number];
  const synthesis = buildDeterministicSynthesis({
    ...
    profileSignals: [
      ...
      signal({
        ...
        confirmation_policy_version: "background-source-summary-tag-confirmation-v1",
        confirmed_at: "2026-06-14T00:00:00.000Z",
        signal_fingerprint: "sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",
        signal_value: "confirmedsourceproposal",
      }),
    ],
    profileSources: [],
  });

  assert.ok(synthesis.capability_tags.includes("confirmedsourceproposal"));
  assert.doesNotMatch(synthesis.capabilities, /unconfirmedsourceproposal/);
});
