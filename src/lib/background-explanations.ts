...
const MAX_DIGEST_PREVIEW_LENGTH = 120;
...
function compactPreviewText(value: string, maxLength = MAX_DIGEST_PREVIEW_LENGTH) {
...
export function buildPrivacySafeMatchAuditSummary({
  score,
  sourceLabel,
}: {
  score: number;
  ...
  return `${sourceLabel} found ${getMatchConfidenceBand(
    score,
  ).toLowerCase()} confidence compatibility as a review prompt; no automatic introduction or moral ranking was issued.`;
}

export function buildPrivacySafeMatchDigestLine({
  publicPreview,
  score,
}: {
  ...
  const preview = compactPreviewText(publicPreview || "Broad preview only");

  return `${preview} (${getMatchConfidenceBand(score).toLowerCase()} confidence; broad-preview only)`;
}
...
export function buildMatchExplanation(input: MatchExplanationInput): MatchExplanation {
  ...
  return {
    ...
    provenance: `Generated by ${
      input.generatedBy || "rule-based scan"
    }; confidence band is a review prompt, not an automatic introduction.`,
