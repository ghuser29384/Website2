...
function isCanonicalProduction() {
  ...
  try {
    return /(^|\.)moraltrade\.org$/i.test(
      new URL(envText("NEXT_PUBLIC_SITE_URL")).hostname,
    );
  } catch {
    return false;
  }
...
export function getDirectDonationUpgradeConfig(): DirectDonationUpgradeConfig {
  const requestedEnabled =
    envText("DIRECT_DONATION_UPGRADES_ENABLED").toLowerCase() === "true";
  const configuredMode = envText("DIRECT_DONATION_UPGRADE_MODE").toLowerCase();
  const mode: DirectDonationUpgradeMode =
    configuredMode === "staging" || configuredMode === "live"
      ? configuredMode
      : "disabled";
  const environment = mode === "disabled" ? null : mode;
  const publicApiKey = envText("EVERY_ORG_PUBLIC_API_KEY");
  ...
  const qaFixturesEnabled =
    envText("DIRECT_DONATION_UPGRADE_QA_FIXTURES").toLowerCase() === "true";
  ...
  if (mode === "staging" && production) {
    blockers.push("Every.org staging is blocked on the canonical production deployment.");
  }
  if (mode === "live" && !production) {
    blockers.push("Live Direct Donation Upgrades are restricted to canonical production.");
  }
...
function qaFixtureIdentities(): EveryOrgNonprofitIdentity[] {
  return [
