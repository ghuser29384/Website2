...
export async function publishPriorityCorrectionCycleForMonth({
  ...
  if ((existingAssignments ?? []).length === 0) {
    ...
    for (const causeArea of causeAreas) {
      ...
      for (const profile of candidates) {
        arbiterPayloads.push({
          ...
          selection_pool: "eligible_cause_participants_random",
          selection_score: 0,
          status: "active",
        });
      }
    ...
    const causeAreaCandidates = profileRows
      .filter((profile) => {
        const snapshot = snapshotsByProfileId.get(profile.id);
        return !recentlyServed.has(profile.id) && snapshot?.prioritized_cause_area;
      })
      .map((profile) => ({
        profile,
    ...
    for (const candidate of pickDiverseCauseAreaArbiters(causeAreaCandidates, 7)) {
      arbiterPayloads.push({
        ...
        cause_area: null,
        selection_pool: "eligible_participants_diverse_random",
        selection_score: 0,
        status: "active",
      });
    }
