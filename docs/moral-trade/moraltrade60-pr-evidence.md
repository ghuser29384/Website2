...
node --import tsx -e 'const m = await import("./src/lib/moral-trade/clearing-previews.ts"); const api = m.default ?? m; const contract = api.getMoralTradeClearingPreviewContract(); const validation = api.validateMoralTradeClearingPreviewContract(contract); console.log(JSON.stringify({ status: validation.status, blockers: validation.blockers, samplePreviews: contract.samplePreviews.map((preview) => ({ track: preview.track, status: preview.status, blockers: preview.blockerCodes.slice(0, 5), captureAllowed: preview.captureAllowed, relianceBearing: preview.relianceBearing })) }, null, 2));'
...
npm run lint -- src/lib/moral-trade/clearing-previews.ts src/lib/moral-trade/clearing-previews.test.ts src/components/offers/offer-create-form.tsx src/app/api/moral-trade/health/route.ts src/app/moral-trade/technical-spec/page.tsx
git diff --check
npm run build
...
- focused clearing-preview contract bundle: `6` tests passed
- clearing-preview broad governance bundle: `193` tests passed
- direct clearing-preview validator probe: `status: pass`, `blockers: []`, sample previews `donation_offset: preview_ready`, `pledge_swap: blocked_preview_only`, both `captureAllowed: false` and `relianceBearing: false`
- lint: passed
