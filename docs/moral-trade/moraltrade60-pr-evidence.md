...
- persistence-backed matching-clearing execution/replay records with an authenticated private no-store POST route, owner-scoped idempotency, RLS, deterministic evaluation hashes, replay hashes, and hard database checks that prevent execution records from creating lock proposals, authorizing payment, authorizing reliance, or publishing public metrics
...
- persistence-backed baseline-integrity enforcement records with an authenticated private no-store POST route, owner-scoped idempotency, RLS, deterministic evaluation hashes, and hard database checks that prevent enforcement records from creating clearable transitions, authorizing payment, authorizing reliance, or publishing public metrics
...
- persistence-backed agreement-amendment enforcement records with an authenticated private no-store POST route, owner-scoped idempotency, RLS, deterministic evaluation hashes, and hard database checks that prevent enforcement records from applying amendments, mutating parent records, allowing material changes, authorizing payment, authorizing reliance, or publishing public metrics
...
- persistence-backed challenge-appeal enforcement records with an authenticated private no-store POST route, owner-scoped idempotency, RLS, deterministic evaluation hashes, and hard database checks that prevent enforcement records from opening appeals, correcting records, authorizing reliance, waiving safety blockers, reopening settled obligations, or publishing public metrics
...
- first-class recipient-acceptance and adverse-association contract/records that keep recipient consent, visible acceptance status, adverse-association review state, risk class, and privacy-leak blockers fail-closed before lock, payment, payout, public metrics, or release promotion
...
- first-class review-capacity and queue-admission contract/records for non-public-goods offers before live, matchable, payable, reliance-bearing, public-metric, or release-gate transitions, including queue overflow, visible user-facing queue status, eligible reviewer/panel assignment, neutral-panel availability, baseline freshness, and payment-authorization freshness gates
...
- first-class financial settlement controls contract/records for platform fees, FX policies/rate snapshots, material notices, server-time deadlines, challenge windows, and payout milestones before real-money previews, locks, authorizations, captures, defaults, payout releases, public metrics, or release promotion
- user-facing donation-offset and pledge-swap clearing previews that keep match candidates non-capture and non-reliance-bearing while showing frozen-run, final-lock, confirmation, ratio-bound, residual, reservation, atomic-settlement, destination, safety, policy, and pledge-performance blockers
- persistence-backed clearing-preview execution records with an authenticated private no-store POST route, owner-scoped idempotency, RLS, preview hashes, and hard database checks that prevent capture, reliance, or deal formation from preview records
...
- `supabase/migrations/20260608_moral_trade_matching_clearing_execution_records.sql`
  - Adds append-only owner-scoped `moral_trade_matching_clearing_execution_records` for matching-clearing evaluation and replay packets, normalized input JSON, deterministic evaluation JSON, replay hashes, blocker codes, user-facing blocker categories, evaluation hashes, and idempotency keys.
  - Enables RLS with owner select/insert policies and hard checks that force `creates_lock_proposal_bool = false`, `payable_transition_allowed_bool = false`, and `reliance_bearing_transition_allowed_bool = false`.
  - Keeps invalid, unauthenticated, unconfigured, duplicate, rate-limited, or insert-failed execution paths from mutating state or authorizing lock proposals, payment, reliance, completion, or metric publication.
...
- `supabase/migrations/20260608_moral_trade_baseline_integrity_enforcement_records.sql`
  - Adds append-only owner-scoped `moral_trade_baseline_integrity_enforcement_records` for normalized baseline-integrity enforcement input JSON, deterministic evaluation JSON, blocker codes, user-facing blocker categories, evaluation hashes, and idempotency keys.
  - Enables RLS with owner select/insert policies and hard checks that force `creates_clearable_transition_bool = false`, `payable_transition_allowed_bool = false`, `reliance_bearing_transition_allowed_bool = false`, and `public_metric_allowed_bool = false`.
  - Keeps invalid, unauthenticated, unconfigured, duplicate, rate-limited, or insert-failed enforcement paths from mutating state or authorizing clearable transitions, payment, reliance, completion, or metric publication.
...
- `supabase/migrations/20260608_moral_trade_agreement_amendment_enforcement_records.sql`
  - Adds append-only owner-scoped `moral_trade_agreement_amendment_enforcement_records` for normalized agreement-amendment enforcement input JSON, deterministic evaluation JSON, blocker codes, user-facing blocker categories, evaluation hashes, and idempotency keys.
  - Enables RLS with owner select/insert policies and hard checks that force `applies_amendment_bool = false`, `material_change_allowed_bool = false`, `parent_record_mutation_allowed_bool = false`, `payment_transition_allowed_bool = false`, `reliance_bearing_transition_allowed_bool = false`, and `public_metric_allowed_bool = false`.
  - Keeps invalid, unauthenticated, unconfigured, duplicate, rate-limited, or insert-failed enforcement paths from mutating state or authorizing amendments, material changes, payment, reliance, completion, or metric publication.
...
- `supabase/migrations/20260611_moral_trade_challenge_appeal_enforcement_records.sql`
  - Adds append-only owner-scoped `moral_trade_challenge_appeal_enforcement_records` for normalized challenge-appeal enforcement input JSON, deterministic appeal-case evaluation JSON, blocker codes, user-facing blocker categories, evaluation hashes, and idempotency keys.
  - Enables RLS with owner select/insert policies and hard checks that force `opens_appeal_bool = false`, `corrects_record_bool = false`, `reliance_bearing_transition_allowed_bool = false`, `safety_blocker_waiver_allowed_bool = false`, `settled_obligation_reopen_allowed_bool = false`, and `public_metric_allowed_bool = false`.
  - Keeps invalid, unauthenticated, unconfigured, duplicate, rate-limited, or insert-failed enforcement paths from opening appeals, correcting records, authorizing reliance, waiving safety blockers, reopening settled obligations, or publishing public metrics.
...
- `supabase/migrations/20260611_moral_trade_recipient_acceptance_records.sql`
  - Adds first-class `moral_trade_recipient_acceptance_policies`, `moral_trade_recipient_acceptance_records`, and `moral_trade_adverse_association_reviews` tables.
  - Extends policy-snapshot subject support for `recipient_acceptance` and `adverse_association`.
  - Keeps missing, pending, declined, revoked, stale, expired, mutable-policy, superseded, severe, unresolved, high-risk unmitigated, insufficiently mitigated, or privacy-leaking acceptance/review evidence fail-closed before recipient listing publication, matched-trade lock, payment authorization, payment capture, payout release, public metric publication, or release-gate promotion.
...
- `supabase/migrations/20260608_moral_trade_financial_settlement_controls.sql`
  - Adds first-class `moral_trade_platform_fee_policies`, `moral_trade_platform_fee_disclosures`, `moral_trade_fx_policies`, `moral_trade_fx_rate_snapshots`, `moral_trade_notification_policies`, `moral_trade_material_notice_records`, `moral_trade_time_authority_policies`, `moral_trade_deadline_records`, `moral_trade_challenge_window_records`, and `moral_trade_payout_milestone_records` tables.
  - Extends policy-snapshot subject support for `challenge_window` and `payout_milestone` alongside existing platform-fee, FX, notification, and time-authority subjects.
  - Keeps missing, mutable, stale, superseded, hidden-fee, stale-FX, metric-inclusion, failed-notice, client-clock, unresolved-challenge, missing-evidence, changed-destination, unverified-destination, or unreleasable milestone evidence fail-closed before money movement, payout release, public metrics, or release-gate promotion.
- `supabase/migrations/20260611_moral_trade_review_capacity_records.sql`
  - Adds first-class `moral_trade_review_capacity_policies`, `moral_trade_review_queue_records`, and `moral_trade_reviewer_panel_assignments` tables.
  - Extends policy-snapshot subject support for `review_capacity` and `review_queue_admission`.
  - Keeps missing, mutable, stale, superseded, overflowing, waitlisted, expired, hidden-status, reviewer-unavailable, neutral-panel-unavailable, baseline-stale, or payment-authorization-stale evidence fail-closed before non-public-goods offers become live, matchable, payable, reliance-bearing, publicly counted, or release-promoted.
- `supabase/migrations/20260608_moral_trade_clearing_preview_records.sql`
  - Adds append-only owner-scoped `moral_trade_clearing_preview_records` for donation-offset and pledge-swap preview execution packets, normalized preview input/result JSON, section statuses, blocker copy, blocker codes, preview hashes, and idempotency keys.
  - Enables RLS with owner select/insert policies and hard checks that force `capture_allowed_bool = false`, `reliance_bearing_bool = false`, `match_candidate_creates_deal_bool = false`, `requires_final_lock_proposal_bool = true`, and `requires_fresh_confirmations_bool = true`.
  - Keeps invalid, unauthenticated, unconfigured, duplicate, rate-limited, or insert-failed execution paths from mutating state or authorizing lock, capture, reliance, completion, or metric inclusion.
...
node --import tsx --test src/lib/moral-trade/matching-clearing.test.ts
node --import tsx --test src/lib/moral-trade/api-contract.test.ts src/lib/public-route-smoke.test.ts src/lib/moral-trade/matching-clearing.test.ts src/lib/moral-trade/clearing-previews.test.ts
node --import tsx --test src/lib/moral-trade/matching-clearing.test.ts src/lib/moral-trade/impact-claims.test.ts src/lib/moral-trade/privacy-governance.test.ts src/lib/moral-trade/anti-enumeration.test.ts src/lib/moral-trade/reviewer-quality.test.ts src/lib/moral-trade/account-security.test.ts src/lib/moral-trade/participant-eligibility.test.ts src/lib/moral-trade/recipient-destination.test.ts src/lib/moral-trade/production-readiness.test.ts src/lib/moral-trade/participant-confirmations.test.ts src/lib/moral-trade/release-gates.test.ts src/lib/moral-trade/api-contract.test.ts src/lib/public-route-smoke.test.ts
node --import tsx --test src/lib/moral-trade/baseline-integrity.test.ts src/lib/moral-trade/matching-clearing.test.ts src/lib/moral-trade/impact-claims.test.ts src/lib/moral-trade/privacy-governance.test.ts src/lib/moral-trade/anti-enumeration.test.ts src/lib/moral-trade/reviewer-quality.test.ts src/lib/moral-trade/account-security.test.ts src/lib/moral-trade/participant-eligibility.test.ts src/lib/moral-trade/recipient-destination.test.ts src/lib/moral-trade/production-readiness.test.ts src/lib/moral-trade/participant-confirmations.test.ts src/lib/moral-trade/release-gates.test.ts src/lib/moral-trade/api-contract.test.ts src/lib/public-route-smoke.test.ts
node --import tsx --test src/lib/moral-trade/baseline-integrity.test.ts
node --import tsx --test src/lib/moral-trade/api-contract.test.ts src/lib/public-route-smoke.test.ts src/lib/moral-trade/baseline-integrity.test.ts src/lib/moral-trade/matching-clearing.test.ts src/lib/moral-trade/clearing-previews.test.ts
node --import tsx --test src/lib/moral-trade/baseline-integrity.test.ts src/lib/moral-trade/matching-clearing.test.ts src/lib/moral-trade/clearing-previews.test.ts src/lib/moral-trade/impact-claims.test.ts src/lib/moral-trade/privacy-governance.test.ts src/lib/moral-trade/anti-enumeration.test.ts src/lib/moral-trade/reviewer-quality.test.ts src/lib/moral-trade/account-security.test.ts src/lib/moral-trade/participant-eligibility.test.ts src/lib/moral-trade/recipient-destination.test.ts src/lib/moral-trade/production-readiness.test.ts src/lib/moral-trade/participant-confirmations.test.ts src/lib/moral-trade/release-gates.test.ts src/lib/moral-trade/agreement-amendments.test.ts src/lib/moral-trade/challenge-appeal.test.ts src/lib/moral-trade/side-agreements.test.ts src/lib/moral-trade/trade-classification.test.ts src/lib/moral-trade/protective-assessments.test.ts src/lib/moral-trade/user-safety-content-moderation.test.ts src/lib/moral-trade/financial-settlement-controls.test.ts src/lib/moral-trade/api-contract.test.ts src/lib/public-route-smoke.test.ts
...
node --import tsx --test src/lib/moral-trade/agreement-amendments.test.ts src/lib/moral-trade/baseline-integrity.test.ts src/lib/moral-trade/matching-clearing.test.ts src/lib/moral-trade/impact-claims.test.ts src/lib/moral-trade/privacy-governance.test.ts src/lib/moral-trade/anti-enumeration.test.ts src/lib/moral-trade/reviewer-quality.test.ts src/lib/moral-trade/account-security.test.ts src/lib/moral-trade/participant-eligibility.test.ts src/lib/moral-trade/recipient-destination.test.ts src/lib/moral-trade/production-readiness.test.ts src/lib/moral-trade/participant-confirmations.test.ts src/lib/moral-trade/release-gates.test.ts src/lib/moral-trade/api-contract.test.ts src/lib/public-route-smoke.test.ts
node --import tsx --test src/lib/moral-trade/api-contract.test.ts src/lib/public-route-smoke.test.ts src/lib/moral-trade/agreement-amendments.test.ts src/lib/moral-trade/baseline-integrity.test.ts src/lib/moral-trade/matching-clearing.test.ts src/lib/moral-trade/clearing-previews.test.ts
node --import tsx --test src/lib/moral-trade/agreement-amendments.test.ts src/lib/moral-trade/baseline-integrity.test.ts src/lib/moral-trade/matching-clearing.test.ts src/lib/moral-trade/clearing-previews.test.ts src/lib/moral-trade/impact-claims.test.ts src/lib/moral-trade/privacy-governance.test.ts src/lib/moral-trade/anti-enumeration.test.ts src/lib/moral-trade/reviewer-quality.test.ts src/lib/moral-trade/account-security.test.ts src/lib/moral-trade/participant-eligibility.test.ts src/lib/moral-trade/recipient-destination.test.ts src/lib/moral-trade/production-readiness.test.ts src/lib/moral-trade/participant-confirmations.test.ts src/lib/moral-trade/release-gates.test.ts src/lib/moral-trade/challenge-appeal.test.ts src/lib/moral-trade/side-agreements.test.ts src/lib/moral-trade/trade-classification.test.ts src/lib/moral-trade/protective-assessments.test.ts src/lib/moral-trade/user-safety-content-moderation.test.ts src/lib/moral-trade/financial-settlement-controls.test.ts src/lib/moral-trade/api-contract.test.ts src/lib/public-route-smoke.test.ts
...
node --import tsx --test src/lib/moral-trade/financial-settlement-controls.test.ts
node --import tsx --test src/lib/moral-trade/financial-settlement-controls.test.ts src/lib/moral-trade/api-contract.test.ts src/lib/public-route-smoke.test.ts
node --import tsx --test src/lib/moral-trade/template-conformance.test.ts src/lib/moral-trade/api-contract.test.ts src/lib/public-route-smoke.test.ts src/lib/moral-trade/challenge-appeal.test.ts src/lib/moral-trade/financial-settlement-controls.test.ts
node --import tsx --test src/lib/moral-trade/review-capacity.test.ts src/lib/moral-trade/api-contract.test.ts src/lib/public-route-smoke.test.ts
node --import tsx --test src/lib/moral-trade/clearing-previews.test.ts
node --import tsx --test src/lib/moral-trade/api-contract.test.ts src/lib/public-route-smoke.test.ts src/lib/moral-trade/clearing-previews.test.ts
node --import tsx --test src/lib/moral-trade/clearing-previews.test.ts src/lib/moral-trade/financial-settlement-controls.test.ts src/lib/moral-trade/user-safety-content-moderation.test.ts src/lib/moral-trade/protective-assessments.test.ts src/lib/moral-trade/trade-classification.test.ts src/lib/moral-trade/side-agreements.test.ts src/lib/proposal-review.test.ts src/lib/moral-trade/challenge-appeal.test.ts src/lib/moral-trade/agreement-amendments.test.ts src/lib/moral-trade/baseline-integrity.test.ts src/lib/moral-trade/matching-clearing.test.ts src/lib/moral-trade/impact-claims.test.ts src/lib/moral-trade/privacy-governance.test.ts src/lib/moral-trade/anti-enumeration.test.ts src/lib/moral-trade/reviewer-quality.test.ts src/lib/moral-trade/account-security.test.ts src/lib/moral-trade/participant-eligibility.test.ts src/lib/moral-trade/recipient-destination.test.ts src/lib/moral-trade/production-readiness.test.ts src/lib/moral-trade/participant-confirmations.test.ts src/lib/moral-trade/release-gates.test.ts src/lib/moral-trade/api-contract.test.ts src/lib/public-route-smoke.test.ts
...
node --import tsx -e 'const m = await import("./src/lib/moral-trade/financial-settlement-controls.ts"); const api = m.default ?? m; const contract = api.getMoralTradeFinancialSettlementControlsContract(); const validation = api.validateMoralTradeFinancialSettlementControlsContract(contract); console.log(JSON.stringify({ status: validation.status, blockers: validation.blockers, sampleEvaluations: contract.sampleEvaluations.map((entry) => ({ transition: entry.transition, status: entry.status, blockers: entry.blockers.slice(0, 5) })) }, null, 2));'
node --import tsx -e 'const m = await import("./src/lib/moral-trade/matching-clearing.ts"); const api = m.default ?? m; const contract = api.getMoralTradeMatchingClearingContract(); const validation = api.validateMoralTradeMatchingClearingContract(contract); console.log(JSON.stringify({ status: validation.status, blockers: validation.blockers, executionTables: contract.executionRecordTables, route: contract.executionRoute, tests: contract.contractTests.filter((test) => test.includes("matching_clearing")) }, null, 2));'
node --import tsx -e 'const routeModule = await import("./src/app/api/moral-trade/matching-clearing/execute/route"); const route = routeModule.POST ? routeModule : routeModule.default; const response = await route.POST(new Request("http://localhost/api/moral-trade/matching-clearing/execute", { method: "POST", body: "not-json" })); console.log(JSON.stringify({ status: response.status, cacheControl: response.headers.get("cache-control"), body: await response.json() }, null, 2));'
node --import tsx -e 'const m = (await import("./src/lib/moral-trade/baseline-integrity")).default; const contract = m.getMoralTradeBaselineIntegrityContract(); const validation = m.validateMoralTradeBaselineIntegrityContract(contract); console.log(JSON.stringify({ status: validation.status, blockers: validation.blockers, enforcementRoute: contract.enforcementRoute, enforcementRecordTables: contract.enforcementRecordTables, contractTests: contract.contractTests.filter((test) => test.includes("enforce") || test.includes("enforcement")) }, null, 2));'
node --import tsx -e 'const routeModule = (await import("./src/app/api/moral-trade/baseline-integrity/enforce/route")).default; const response = await routeModule.POST(new Request("http://localhost/api/moral-trade/baseline-integrity/enforce", { method: "POST", body: "not-json" })); console.log(JSON.stringify({ status: response.status, cacheControl: response.headers.get("cache-control"), body: await response.json() }, null, 2));'
node --import tsx -e 'const m = (await import("./src/lib/moral-trade/agreement-amendments")).default; const contract = m.getMoralTradeAgreementAmendmentContract(); const validation = m.validateMoralTradeAgreementAmendmentContract(contract); console.log(JSON.stringify({ status: validation.status, blockers: validation.blockers, enforcementRoute: contract.enforcementRoute, enforcementRecordTables: contract.enforcementRecordTables, contractTests: contract.contractTests.filter((test) => test.includes("enforce") || test.includes("enforcement")) }, null, 2));'
node --import tsx -e 'const routeModule = (await import("./src/app/api/moral-trade/agreement-amendments/enforce/route")).default; const response = await routeModule.POST(new Request("http://localhost/api/moral-trade/agreement-amendments/enforce", { method: "POST", body: "not-json" })); const body = await response.json(); console.log(JSON.stringify({ status: response.status, cacheControl: response.headers.get("cache-control"), ok: body.ok, stateMutation: body.stateMutation, appliesAmendment: body.appliesAmendment, materialChangeAllowed: body.materialChangeAllowed, parentRecordMutationAllowed: body.parentRecordMutationAllowed, paymentTransitionAllowed: body.paymentTransitionAllowed, relianceBearingTransitionAllowed: body.relianceBearingTransitionAllowed, publicMetricAllowed: body.publicMetricAllowed, persistence: body.persistence, blockers: body.blockers }, null, 2));'
node --import tsx -e 'const m = await import("./src/lib/moral-trade/clearing-previews.ts"); const api = m.default ?? m; const contract = api.getMoralTradeClearingPreviewContract(); const validation = api.validateMoralTradeClearingPreviewContract(contract); console.log(JSON.stringify({ status: validation.status, blockers: validation.blockers, tables: contract.firstClassRecordTables, route: contract.executionRoute, tests: contract.contractTests.filter((test) => test.includes("clearing_preview")) }, null, 2));'
node --import tsx -e 'const routeModule = await import("./src/app/api/moral-trade/clearing-previews/execute/route"); const route = routeModule.POST ? routeModule : routeModule.default; const response = await route.POST(new Request("http://localhost/api/moral-trade/clearing-previews/execute", { method: "POST", body: "not-json" })); console.log(JSON.stringify({ status: response.status, cacheControl: response.headers.get("cache-control"), body: await response.json() }, null, 2));'
node --import tsx -e 'const m = (await import("./src/lib/moral-trade/challenge-appeal.ts")).default; const contract = m.getMoralTradeChallengeAppealContract(); const validation = m.validateMoralTradeChallengeAppealContract(contract); console.log(JSON.stringify({ status: validation.status, blockers: validation.blockers, enforcementRoute: contract.enforcementRoute, enforcementRecordTables: contract.enforcementRecordTables, contractTests: contract.contractTests.filter((test) => test.includes("enforce") || test.includes("enforcement")) }, null, 2));'
node --import tsx -e 'const routeModule = (await import("./src/app/api/moral-trade/challenge-appeal/enforce/route.ts")).default; const response = await routeModule.POST(new Request("http://localhost/api/moral-trade/challenge-appeal/enforce", { method: "POST", body: "not-json" })); const body = await response.json(); console.log(JSON.stringify({ status: response.status, cacheControl: response.headers.get("cache-control"), ok: body.ok, stateMutation: body.stateMutation, opensAppeal: body.opensAppeal, correctsRecord: body.correctsRecord, relianceBearingTransitionAllowed: body.relianceBearingTransitionAllowed, safetyBlockerWaiverAllowed: body.safetyBlockerWaiverAllowed, settledObligationReopenAllowed: body.settledObligationReopenAllowed, publicMetricAllowed: body.publicMetricAllowed, persistence: body.persistence, blockers: body.blockers }, null, 2));'
...
npm run lint -- src/lib/moral-trade/matching-clearing.ts src/lib/moral-trade/matching-clearing.test.ts src/app/api/moral-trade/matching-clearing/contract/route.ts src/app/api/moral-trade/matching-clearing/execute/route.ts src/app/api/moral-trade/health/route.ts src/app/moral-trade/technical-spec/page.tsx src/lib/moral-trade/api-contract.ts src/lib/moral-trade/api-contract.test.ts src/lib/moral-trade/api-rate-limit.ts src/lib/public-route-smoke.test.ts src/lib/moral-trade/financial-settlement-controls.test.ts src/lib/supabase/database.types.ts
...
npm run lint -- src/lib/moral-trade/baseline-integrity.ts src/lib/moral-trade/baseline-integrity.test.ts src/app/api/moral-trade/baseline-integrity/contract/route.ts src/app/api/moral-trade/baseline-integrity/enforce/route.ts src/app/api/moral-trade/health/route.ts src/app/moral-trade/technical-spec/page.tsx src/lib/moral-trade/api-rate-limit.ts src/lib/moral-trade/api-contract.ts src/lib/moral-trade/api-contract.test.ts src/lib/public-route-smoke.test.ts src/lib/moral-trade/financial-settlement-controls.test.ts src/lib/supabase/database.types.ts
npm run lint -- src/lib/moral-trade/agreement-amendments.ts src/lib/moral-trade/agreement-amendments.test.ts src/app/api/moral-trade/agreement-amendments/contract/route.ts src/app/api/moral-trade/agreement-amendments/enforce/route.ts src/app/api/moral-trade/health/route.ts src/app/moral-trade/technical-spec/page.tsx src/lib/moral-trade/api-rate-limit.ts src/lib/moral-trade/api-contract.ts src/lib/moral-trade/api-contract.test.ts src/lib/public-route-smoke.test.ts src/lib/moral-trade/financial-settlement-controls.test.ts src/lib/supabase/database.types.ts config/moral-trade/api-contract-profile.json
npm run lint -- src/lib/moral-trade/challenge-appeal.ts src/lib/moral-trade/challenge-appeal.test.ts src/app/api/moral-trade/challenge-appeal/contract/route.ts src/app/api/moral-trade/challenge-appeal/enforce/route.ts src/app/api/moral-trade/health/route.ts src/app/moral-trade/technical-spec/page.tsx src/lib/moral-trade/api-rate-limit.ts src/lib/moral-trade/api-contract.ts src/lib/moral-trade/api-contract.test.ts src/lib/public-route-smoke.test.ts src/lib/moral-trade/financial-settlement-controls.test.ts src/lib/supabase/database.types.ts config/moral-trade/api-contract-profile.json
...
npm run lint -- src/lib/moral-trade/financial-settlement-controls.ts src/lib/moral-trade/financial-settlement-controls.test.ts src/app/api/moral-trade/financial-settlement-controls/contract/route.ts src/app/api/moral-trade/health/route.ts src/app/moral-trade/technical-spec/page.tsx src/lib/moral-trade/api-contract.ts src/lib/moral-trade/api-contract.test.ts src/lib/public-route-smoke.test.ts src/lib/supabase/database.types.ts config/moral-trade/api-contract-profile.json
npm run lint -- src/lib/moral-trade/template-conformance.ts src/lib/moral-trade/template-conformance.test.ts src/app/api/moral-trade/template-conformance/contract/route.ts src/app/api/moral-trade/health/route.ts src/app/moral-trade/technical-spec/page.tsx src/lib/moral-trade/api-contract.ts src/lib/moral-trade/api-contract.test.ts src/lib/public-route-smoke.test.ts src/lib/moral-trade/challenge-appeal.test.ts src/lib/moral-trade/financial-settlement-controls.test.ts src/lib/supabase/database.types.ts config/moral-trade/api-contract-profile.json
npm run lint -- src/lib/moral-trade/review-capacity.ts src/lib/moral-trade/review-capacity.test.ts src/app/api/moral-trade/review-capacity/contract/route.ts src/app/api/moral-trade/health/route.ts src/app/moral-trade/technical-spec/page.tsx src/lib/moral-trade/api-contract.ts src/lib/moral-trade/api-contract.test.ts src/lib/public-route-smoke.test.ts src/lib/supabase/database.types.ts config/moral-trade/api-contract-profile.json
npm run lint -- src/lib/moral-trade/clearing-previews.ts src/lib/moral-trade/clearing-previews.test.ts src/app/api/moral-trade/clearing-previews/execute/route.ts src/app/api/moral-trade/health/route.ts src/app/moral-trade/technical-spec/page.tsx src/lib/moral-trade/api-contract.ts src/lib/moral-trade/api-contract.test.ts src/lib/moral-trade/api-rate-limit.ts src/lib/public-route-smoke.test.ts src/lib/moral-trade/financial-settlement-controls.test.ts src/lib/supabase/database.types.ts
git diff --check
npm run build
...
- focused matching-clearing contract/execution bundle: `7` tests passed
- matching-clearing/API/source-smoke/clearing-preview bundle: `69` tests passed
- matching-clearing broad governance bundle: `196` tests passed
- direct matching-clearing validator probe: `status: pass`, `blockers: []`, execution table `moral_trade_matching_clearing_execution_records`, execution route `POST /api/moral-trade/matching-clearing/execute`, authenticated append-only execution record
- direct matching-clearing route probe: invalid JSON returned `400`, `Cache-Control: private, no-store`, `stateMutation: false`, `createsLockProposal: false`, `payableTransitionAllowed: false`, `relianceBearingTransitionAllowed: false`, persistence status `not_recorded`, and blocker `invalid_json_body`
- focused baseline-integrity contract/enforcement bundle: `6` tests passed
- baseline-integrity/API/source-smoke/matching-clearing/clearing-preview bundle: `75` tests passed
- baseline-integrity endpoint-enforcement broad governance bundle: `180` tests passed
- direct baseline-integrity validator probe: `status: pass`, `blockers: []`, enforcement table `moral_trade_baseline_integrity_enforcement_records`, enforcement route `POST /api/moral-trade/baseline-integrity/enforce`, authenticated append-only enforcement record
- direct baseline-integrity enforce route probe: invalid JSON returned `400`, `Cache-Control: private, no-store`, `stateMutation: false`, `createsClearableTransition: false`, `payableTransitionAllowed: false`, `relianceBearingTransitionAllowed: false`, `publicMetricAllowed: false`, persistence status `not_recorded`, and blocker `invalid_json_body`
- focused agreement-amendment contract/enforcement bundle: `6` tests passed
- agreement-amendment/API/source-smoke/baseline-integrity/matching-clearing/clearing-preview bundle: `81` tests passed
- agreement-amendment endpoint-enforcement broad governance bundle: `181` tests passed
- direct agreement-amendment validator probe: `status: pass`, `blockers: []`, enforcement table `moral_trade_agreement_amendment_enforcement_records`, enforcement route `POST /api/moral-trade/agreement-amendments/enforce`, authenticated append-only enforcement record
- direct agreement-amendment enforce route probe: invalid JSON returned `400`, `Cache-Control: private, no-store`, `stateMutation: false`, `appliesAmendment: false`, `materialChangeAllowed: false`, `parentRecordMutationAllowed: false`, `paymentTransitionAllowed: false`, `relianceBearingTransitionAllowed: false`, `publicMetricAllowed: false`, persistence status `not_recorded`, and blocker `invalid_json_body`
- focused challenge-appeal contract/enforcement bundle: `13` tests passed
- challenge-appeal/API/source-smoke bundle: `67` tests passed
...
- challenge-appeal endpoint-enforcement broad governance bundle: `183` tests passed
- direct challenge-appeal validator probe: `status: pass`, `blockers: []`, enforcement table `moral_trade_challenge_appeal_enforcement_records`, enforcement route `POST /api/moral-trade/challenge-appeal/enforce`, authenticated append-only enforcement record
- direct challenge-appeal enforce route probe: invalid JSON returned `400`, `Cache-Control: private, no-store`, `stateMutation: false`, `opensAppeal: false`, `correctsRecord: false`, `relianceBearingTransitionAllowed: false`, `safetyBlockerWaiverAllowed: false`, `settledObligationReopenAllowed: false`, `publicMetricAllowed: false`, persistence status `not_recorded`, and blocker `invalid_json_body`
...
- focused financial-settlement-controls contract bundle: `7` tests passed
- financial-settlement-controls API/source-smoke bundle: `61` tests passed
- financial-settlement-controls broad governance bundle: `187` tests passed
- direct financial-settlement-controls validator probe: `status: pass`, `blockers: []`, sample evaluations `draft_preview: pass`, `matched_trade_lock: pass`, `payout_milestone_release: blocked`
- focused review-capacity/API/source-smoke bundle: `62` tests passed
- review-capacity broad governance bundle: `208` tests passed
- direct review-capacity validator probe: `status: pass`, `blockers: []`, sample evaluations cover `draft_preview: pass`, `matchable_publication: pass`, waitlisted overflow blocking, payment-authorization staleness blocking, and neutral-panel blocking
- focused clearing-preview contract/execution bundle: `8` tests passed
- clearing-preview API/source-smoke bundle: `62` tests passed
- clearing-preview broad governance bundle: `195` tests passed
- direct clearing-preview validator probe: `status: pass`, `blockers: []`, first-class table `moral_trade_clearing_preview_records`, execution route `POST /api/moral-trade/clearing-previews/execute`, authenticated append-only preview record
- direct clearing-preview route probe: invalid JSON returned `400`, `Cache-Control: private, no-store`, `stateMutation: false`, `captureAllowed: false`, `relianceBearing: false`, persistence status `not_recorded`, and blocker `invalid_json_body`
- lint: passed
- lint command note: the first attempted `--file` invocation failed because this repo uses ESLint flat config; the successful lint gate used explicit path arguments, and the npm script prepended `eslint .`.
...
Template-conformance contract sample:
...
```json
{
  ...
  "sampleEvaluations": [
    ...
    },
    {
      "transition": "matched_trade_lock",
      "status": "pass",
      "conformingInstanceCount": 1,
      "blockers": []
    },
    {
      "transition": "reliance_bearing_transition",
      "status": "pass",
      "offTemplateExceptionCount": 1,
      "blockers": []
    },
    {
      "transition": "payment_capture",
      ...
      "blockers": [
        ...
        "free_text_creates_side_payments:template-instance:demo",
...
Review-capacity contract sample:
...
```json
{
  ...
  "sampleEvaluations": [
    ...
    },
    {
      ...
      "status": "pass",
      "admittedQueueCount": 1,
      "eligiblePanelCount": 1,
      "blockers": []
    },
    {
      ...
      "blockers": [
        ...
        "review_queue_over_policy_depth:review-queue:offset-offer-demo",
    ...
    },
    {
      ...
      "status": "blocked",
      "blockers": [
        "estimated_review_after_baseline_expiry:review-queue:offset-offer-demo",
...
AI preference-elicitation boundary slice:

- Added `moral_trade_ai_preference_elicitation_policies` and `moral_trade_ai_preference_elicitation_records` with immutable policy references, hash-backed AI output and user-edited structured input references, prohibited hidden willingness-to-pay inference, prohibited autonomous counteroffer/acceptance, prohibited AI state changes, and privacy fail-closed checks.
- Added `/api/moral-trade/ai-preference-elicitation/contract` plus aggregate health/spec/API-catalog wiring for AI preference-elicitation scopes, states, transitions, first-class tables, policy subjects, sample evaluations, and prohibited-use blockers.
- Extended clearing previews to surface recipient acceptance, adverse-association, and AI preference-elicitation boundary statuses so missing or non-passing boundary controls keep previews non-reliance-bearing.
- Verified with focused contract/smoke tests, lint, production build, and runtime route probes against `/api/moral-trade/ai-preference-elicitation/contract` and `/api/moral-trade/health`.
...
- The matching-clearing execute route is authenticated and private no-store; invalid or unauthenticated requests do not write records, and successful execution records remain owner-scoped append-only evidence that cannot create lock proposals, authorize payment, authorize reliance, publish public metrics, or expose raw input bundles, private counterparty data, hidden match reasoning, reviewer notes, or participant-specific final confirmations.
...
- The baseline-integrity enforce route is authenticated and private no-store; invalid or unauthenticated requests do not write records, and successful enforcement records remain owner-scoped append-only evidence that cannot create clearable transitions, authorize payment, authorize reliance, publish public metrics, or expose raw baseline narratives, private evidence, exact private constraints, counterparty-specific timing, reviewer notes, or participant-specific assessments.
...
- The agreement-amendment enforce route is authenticated and private no-store; invalid or unauthenticated requests do not write records, and successful enforcement records remain owner-scoped append-only evidence that cannot apply amendments, mutate parent records, allow material changes, authorize payment, authorize reliance, publish public metrics, or expose private amendment narratives, participant identities, confirmation payloads, reviewer notes, payment details, private baselines, or counterparty-specific terms.
...
- The challenge-appeal enforce route is authenticated and private no-store; invalid or unauthenticated requests do not write records, and successful enforcement records remain owner-scoped append-only evidence that cannot open appeals, correct records, authorize reliance, waive safety blockers, reopen settled obligations, publish public metrics, or expose private appeal narratives, appellant identities, raw evidence, reviewer notes, safety-sensitive details, or counterparty-specific dispute facts.
...
- The recipient-acceptance route publishes static transition/table/status/risk-class/sample-status contract metadata; it does not expose recipient private notes, donor private terms, raw adverse-association evidence, expanded recipient identities, private donor reasons, reviewer notes, payment details, raw provider payloads, or participant-specific acceptance/review records.
- The AI preference-elicitation route publishes static transition/table/scope/state/sample-status contract metadata; it does not expose raw prompts, raw AI outputs, hidden willingness-to-pay estimates, hidden negotiation moves, private participant notes, reviewer notes, private disclosure candidates, payment details, or participant-specific elicitation records.
...
- The template-conformance route publishes static transition/table/status/trade-type/sample-status contract metadata; it does not expose private terms, exact caps, free-text narratives, hidden counterparty data, reviewer notes, private wishes, payment details, or participant-specific template instance records.
- The review-capacity route publishes static transition/table/status/queue-status/panel-state/sample-status contract metadata; it does not expose reviewer identities, conflict facts, private queue reasons, participant-specific queue records, baseline details, payment authorization details, reviewer notes, source evidence, contact details, or raw internal status copies.
- The participant-term-sheet route publishes static transition/table/status/term-sheet/disclosure/sample-status contract metadata; it does not expose participant-specific term sheets, raw counterparty identities, contact details, private wishes, exact constraints, hidden match reasoning, source evidence, reviewer notes, payment details, or participant-specific staged disclosure records.
...
- The clearing-preview execute route is authenticated and private no-store; invalid or unauthenticated requests do not write records, and successful records remain owner-scoped append-only preview records that do not expose private counterparty records, raw input bundles, hidden match reasoning, payment credentials, reviewer notes, or participant-specific final confirmations.
- The health, measurement, release-gate, participant-confirmation, participant-eligibility, account-security, reviewer-quality, anti-enumeration, privacy-governance, impact-claim, matching-clearing, clearing-preview, baseline-integrity, agreement-amendment, challenge-appeal, production-readiness, recipient-destination, recipient-acceptance, AI preference-elicitation, side-agreement, trade-classification, template-conformance, review-capacity, participant-term-sheet, and protective-assessment surfaces publish validator status and aggregate contract metadata, not private participant records, participant-specific discovery records, participant-specific impact-claim records, raw matching bundles, participant-specific final confirmations, participant-specific baseline assessments, participant-specific baseline enforcement rows, participant-specific amendment records, participant-specific amendment enforcement rows, participant-specific appeal records, participant-specific appeal enforcement rows, participant-specific destination records, participant-specific acceptance/review records, participant-specific AI elicitation records, participant-specific side-agreement records, participant-specific classification records, participant-specific template instance records, participant-specific queue records, participant-specific term-sheet records, participant-specific staged disclosure records, participant-specific assessment records, or private operational evidence.
...
- `moraltrade60.md` and `moraltrade68.md` include broader long-tail requirements beyond this PR slice, including live operational execution for the new production-readiness, participant-eligibility, account-security, reviewer-quality, anti-enumeration, privacy-governance, impact-claim, matching-clearing, baseline-integrity, agreement-amendment, challenge-appeal, recipient/destination, recipient-acceptance, AI preference-elicitation, side-agreement, trade-classification, template-conformance, review-capacity, and participant-term-sheet records, live lock-proposal creation jobs, live endpoint enforcement for privacy access logs, live endpoint enforcement for impact-claim publication records, live endpoint enforcement for anti-enumeration logs/probe audits, live endpoint enforcement for recipient-acceptance records and adverse-association review records before lock/payment/payout/public metrics, live endpoint enforcement for AI preference-elicitation records before AI-shaped baselines/caps/cause buckets/side constraints/evidence preferences/fallback rules can affect matching, clearing, disclosure, payment, public metrics, or release promotion, live endpoint enforcement for side-agreement disclosure/review records before reliance-bearing transitions, live endpoint enforcement for trade-classification and compensated-action-term records before moral-trade metric publication or payable/reliance-bearing transitions, live endpoint enforcement for template-instance records before live publication, lock, payment, reliance, or public metrics, live endpoint enforcement for queue-admission records before live/matchable/payable/reliance-bearing transitions, live endpoint enforcement for participant-term-sheet and staged-counterparty-disclosure records before live/matchable/payable/reliance-bearing transitions, reviewer audit sampling execution, and live deployment exercise of matching-clearing, clearing-preview, baseline-integrity, agreement-amendment, challenge-appeal, and AI preference-elicitation enforcement against production Supabase/authenticated traffic.
