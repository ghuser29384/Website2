...
8. Before scoring or surfacing any candidate, the helper applies a candidate-side exposure gate: the candidate profile must be active, not paused or deleted, and its current discoverability / inbound-delegate settings must authorize this delegate's audience scope, cohort, and allowed surfaces.
...
8. Before scoring or surfacing any candidate, the helper run enforces candidate-side exposure consent: the candidate profile must be active and its current discoverability / inbound-delegate settings must authorize this delegate's audience scope, cohort, and allowed surfaces.
...
- Candidate-side exposure consent is mandatory. A candidate profile may be scored or surfaced only if the candidate's current profile is active, not paused or deleted, and its discoverability / inbound-delegate settings authorize the requesting delegate's audience scope, cohort, and allowed surfaces. `owner_only`, out-of-cohort, paused, deleted, revoked, or expired candidate exposure states are hard blockers.
...
| `inbound_delegate_discovery` | enum: `off \| cohort_only \| partner_matchmaker \| public_broad_preview` |
| `inbound_delegate_surfaces` | `text[] not null default '{broad_profile}'` |
...
- `inbound_delegate_surfaces` must be limited to broad-preview-safe fields unless a later consent stage explicitly grants more.
...
- After the retention window and any safety-review hold, replace linkable candidate references with non-linkable redacted state, delete or anonymize free-text feedback, and keep only aggregate/redacted audit metadata.
...
- candidate profiles with `opt_in_status` off, paused, deleted, or owner-only are not scored or surfaced
...
- Candidate profiles are scored or surfaced only when the candidate's current exposure settings authorize delegate-mediated discovery for the relevant audience scope, cohort, and broad surfaces.
...
- Linkable candidate references and free-text artifacts have explicit retention windows and are deleted or anonymized after participant review, safety-review holds, or legal obligations no longer require them.
...
- Delegate runs, opportunity briefs, feedback, intro requests, and receipts have explicit retention windows; after those windows, linkable candidate references and free-text content are deleted or anonymized while only permitted redacted/anonymized audit rows remain.
