# Coalition-Routed Escrowed Conditional Matching (CRECM v1.4)
...
Build **Coalition-Routed Escrowed Conditional Matching v1.4** in `moraltrade.org`.

- **Abbreviation:** CRECM
- **User-facing label:** Common Ground Budget
- **Technical label:** CRECM v1.4
...
Reject or clamp any persisted identity weight outside `0..10_000` basis points before the round can lock. A paused, expired, canceled, or consent-invalid budget contributes zero in all three accounting channels and must not be routed, authorized, or captured. Dissent allocates no money but increments `dissentPressure`.
...
```env
MPGF_MECHANISM_VERSION=crecm_v1_4
```
...
```text
Coalition-Routed Escrowed Conditional Matching v1.4
```
...
> **CRECM v1.4: Common Ground Budget + cross-view conditional clearing + supervised escrow/custody + hard review gates + separated actual/counted/match-eligible accounting + active-budget consent gating + enforced identity thresholds + sponsor-pool precommitment + base match + stance-consistent capped diversity-aware post-clear bonus + explicit failure handling.**
