...
- Conditional owner trajectory versus probability-weighted adoption/operational failure.
- Eleven separately identified structural scenarios from `SCENARIOS.json`.
- DAC coverage controller, stress-percentile policy, and every fixed integer surcharge rate from 2% through 15%.
- Linear failure-bonus schedules 5% to 1%, 10% to 2%, 15% to 2%, plus a front-loaded nonlinear 10% to 2% curve.
- Central 12-month displacement versus 90-day early sensitivity.
- Conditionally recoverable $2,500 founder reserve versus permanent-cost treatment.
- Monte Carlo convergence at 2,000, 20,000, and 200,000 draws.

`outputs/parameter_correlations.csv` ranks linear associations between uncertain inputs and cumulative net causal cash. `outputs/variance_drivers.csv` reports normalized squared-correlation screening weights, not causal or Sobol decompositions. Nonlinearity and interactions mean the screening weights need not capture all variance.

## Pre-results dominant cruxes
