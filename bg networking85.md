...
## Revision 85 improvements

This revision adds public-page UX and language simplification instructions based on review of the current Moral Trade background-networking public surface (`/background-networking`, `/wish-registry`, `/api/moral-trade/match-signal/contract`, `/api/moral-trade/private-overlap/contract`, `/privacy`, `/safety`, `/measurement`, `/transparency`, `/accessibility`, and the technical-spec background-networking contract rows):

- **Public-page two-layer information architecture.** The current public background-networking route correctly states broad previews first, consent before detail, no autonomous outreach, no private-feed mining, minimal telemetry, anti-enumeration budgets, and staged disclosure, but it exposes too many implementation sections at the same level. Public routes should therefore default to a short task-based explanation and move contract status, route health, factor-code lists, table names, validator rows, and detailed policy mechanics behind clearly labelled “Technical details” panels or dedicated contract links. This simplifies comprehension while preserving all evidence, transparency, and auditability.
- **Single public mental model across all related pages.** The public pages should use one consistent five-step model: `Create broad profile` → `Choose who can look` → `Get a possible opportunity` → `Ask to explore` → `Share exact details only if both sides agree`. All related pages must repeat the same distinction between a public/broad preview, a possible opportunity, an introduction request, mutual consent, and exact field disclosure.
- **Page-specific simplification without deleting safety content.** `/background-networking` should become the primary plain-language explainer; `/wish-registry` should become a broad-preview browsing page; `/privacy`, `/safety`, `/measurement`, `/transparency`, and `/accessibility` should expose concise background-networking summaries first and place full inventories, source-table names, validator details, and technical contract evidence behind technical-detail toggles. JSON contract routes remain available for implementers but should not be the default user reading path.
- **Plain-language route copy contract.** Public copy should replace implementation phrases such as `deterministic matching`, `purpose-bound grants`, `anti-enumeration budgets`, `redacted match-signal preview`, `private-overlap computation`, `DPIA`, `RLS`, `contract check`, `blocker`, and `source table` with governed plain-language labels in default views, while preserving exact terms in technical panels and public contracts.
- **Simple page templates and copy budgets.** Each related public page should use a predictable structure: one-sentence promise, three safety cards (`What it can use`, `What stays hidden`, `What you control`), a short step list, one primary action, one secondary technical link, and no more than one visible contract/status block before the fold. Copy budgets, jargon budgets, and accessibility/readability checks should be promotion gates for these pages.
- **Non-compromising simplification invariant for public pages.** Simplified pages must still visibly disclose: broad previews before exact details, separate outbound/inbound consent, no autonomous outreach, no raw private-feed mining, no hidden ML ranking of people, human/operator review before disclosure/contact, generic opportunity states, revocation/freeze/delete/export/correction paths, privacy-safe analytics, and links to technical contracts. If a plain-language page cannot safely compress a concept, it must show the technical-detail panel rather than omit the concept.
...
- Public Background Networking pages must be simplified without becoming incomplete. The default public reading path for `/background-networking`, `/wish-registry`, `/privacy`, `/safety`, `/measurement`, `/transparency`, `/accessibility`, and related technical-spec/contract links must use the shared five-step mental model, plain-language term map, compact safety cards, and technical-detail disclosure pattern. Public simplification must not hide or weaken broad-preview boundaries, separate outbound/inbound consent, no-autonomous-outreach boundaries, no-private-feed-mining boundaries, no-hidden-ranking boundaries, human-review-before-disclosure boundaries, revocation/freeze/delete/export/correction paths, small-count suppression, or links to exact technical contracts. Raw JSON, route matrices, table names, validator names, and implementation jargon belong behind technical-detail links, not in the default novice path.
...
| `copy_surface` | enum: `plain_language_term_map | onboarding | consent_dialog | privacy_impact_summary | effective_permission_preview | opportunity_card | intro_prompt | counterparty_prompt | notification | receipt | unavailable_state | correction_export | privacy_freeze | accessibility_label | docs_excerpt | public_background_page` |
...
## `background_public_background_page_simplification_specs`

Use an existing docs/copy-governance system if present; otherwise include this as a typed configuration bundle under the active UI-copy bundle. This table governs the simplified default public reading path for Background Networking-related pages while preserving full technical details behind explicit links.

| Field | Type / constraint |
|---|---|
| `id` | `uuid primary key` or stable typed-config id |
| `ui_copy_bundle_version` | `text not null` |
| `ui_copy_bundle_hash` | `text not null`; canonical content hash of the UI-copy bundle containing this public-page spec |
| `route_path` | `text not null`; e.g. `/background-networking`, `/wish-registry`, `/privacy`, `/safety`, `/measurement`, `/transparency`, `/accessibility`, `/moral-trade/technical-spec` |
| `page_role` | enum: `primary_explainer | broad_preview_browser | trust_summary | measurement_summary | transparency_summary | accessibility_summary | technical_contract_index` |
| `default_reader_task` | `text not null`; plain-language task the page serves |
| `shared_flow_step_keys` | `text[] not null`; ordered subset of the five public mental-model steps |
| `required_safety_card_keys` | `text[] not null`; e.g. `what_it_can_use`, `what_stays_hidden`, `what_you_control` |
| `primary_cta_key` | `text nullable`; governed UI-copy key |
| `secondary_cta_key` | `text nullable`; governed UI-copy key |
| `technical_detail_link_keys` | `text[] not null default '{}'`; exact contract/docs links that must remain available |
| `required_visible_boundary_keys` | `text[] not null`; boundaries that must appear in the default view or an immediately visible safety card |
| `default_view_copy_budget` | `jsonb not null`; max default sections/cards/status blocks/jargon terms before technical details |
| `forbidden_default_terms` | `text[] not null default '{}'`; implementation jargon, raw factor codes, raw contract/check labels, and unsafe euphemisms forbidden in the novice path |
| `technical_detail_panel_required` | `boolean not null default true` |
| `reading_level_state` | enum: `not_checked | passed | failed` |
| `accessibility_state` | enum: `not_checked | passed | failed` |
| `localization_review_state` | enum: `not_required | pending | passed | failed` |
| `page_spec_record_hash` | `text not null`; hash of this page-spec row's canonical policy fields and copy references |
| `status` | enum: `draft | active | deprecated | disabled` |
| `created_at` | `timestamptz` |
| `activated_at` | `timestamptz nullable` |

Public-page simplification requirements:

- Every public route that explains Background Networking must have an active page-spec row or repository-equivalent typed config under the active UI-copy bundle before promotion. Missing, disabled, wrong-locale, stale-hash, failed-reading-level, failed-accessibility, or failed-localization rows fail the public-page promotion gate.
- Public page specs must preserve the feature and safety boundaries listed in this document. A route may simplify wording, layout, and default information hierarchy, but it must not remove required disclosures or hide the only path to technical contracts.
- Default views must render from the active plain-language term map and public-page spec. Technical-detail panels may expose exact factor codes, JSON contracts, validator rows, source-table names, route matrices, and implementation details only when those details are non-disclosing and already public.
- Public routes must not render raw JSON, route matrices, table names, validator names, or raw factor-code strings in the default novice path unless the active page spec explicitly marks the route as a technical contract index.
- CI and promotion tests must compare `/background-networking`, `/wish-registry`, `/privacy`, `/safety`, `/measurement`, `/transparency`, `/accessibility`, related technical-spec sections, and contract-link summaries against the active public-page specs. Stale docs, missing technical links, unsafe jargon in default copy, omitted safety boundaries, raw JSON in novice views, inconsistent terms, or copy-budget violations fail closed before promotion.
...
# Public Background Networking Pages and Language

Codex must simplify the public moraltrade.org background-networking reading path without removing any safety, transparency, or contract evidence. Treat the public pages as a two-layer interface:

1. **Default public explanation:** short, task-based, plain-language, non-technical, and safe for first-time users.
2. **Technical details:** complete contract evidence, validator status, factor codes, source-table names, JSON contracts, policy terms, rollout gates, and developer-facing implementation details.

A reader should understand the safe mental model in under one minute without reading JSON, validator rows, table names, bundle names, or implementation jargon. A reviewer or implementer must still be able to reach the exact contracts and technical evidence with one explicit “Technical details” or “View contract” action.

## Public-route simplification targets

Apply this to all current and future public Background Networking-related routes, including at minimum:

- `/background-networking`
- `/wish-registry`
- `/api/moral-trade/match-signal/contract` when linked from user-facing pages
- `/api/moral-trade/private-overlap/contract` when linked from user-facing pages
- `/privacy` background-networking sections
- `/safety` background-networking sections
- `/measurement` background-networking sections
- `/transparency` background-networking sections
- `/accessibility` background-networking sections
- `/moral-trade/technical-spec` rows and links related to background networking, private matching, disclosure, anti-enumeration, privacy controls, AI shadow mode, notifications, route health, and document coverage

Do not remove technical routes, JSON contracts, public validators, or source evidence. Instead, stop making them the default explanatory layer for ordinary users.

## Shared public mental model

Every public route that explains background networking must use the same plain-language sequence:

1. **Create a broad profile.** The user writes high-level interests, offers, asks, constraints, and verification preferences.
2. **Choose who can look.** The user separately controls whether Moral Trade may look for opportunities for them and whether others may find their broad preview.
3. **Get a possible opportunity.** A possible opportunity is only a privacy-safe brief, not an introduction, ranking, endorsement, or contact event.
4. **Ask to explore.** A user can ask for operator-reviewed next steps; the other side is not exposed to exact details before the relevant consent stage.
5. **Share exact details only if both sides agree.** Contact details, exact wishes, source notes, sensitive constraints, and private evidence move only through explicit field-level disclosure grants after the configured review/consent stage.

The same labels and ordering should appear on `/background-networking`, `/wish-registry`, dashboard entry points, privacy controls, safety explanations, and public transparency summaries.

## `/background-networking` rewrite instructions

Make `/background-networking` the primary plain-language explainer. The default page should be short and should not read like a technical audit. Use this structure:

1. **Hero:** “Find possible trades without exposing private details.”
   - One sentence: “Moral Trade can compare broad previews and saved preferences, then show a privacy-safe possible opportunity for you to review.”
   - Primary CTA: “Create a broad profile” or “Go to dashboard.”
   - Secondary CTA: “Browse broad previews.”
   - Technical CTA: “View technical contracts.”
2. **Three safety cards:**
   - `Broad first`: broad cause areas, trade mode, coarse constraints, and verification preferences can be compared.
   - `Exact details stay hidden`: exact wishes, source notes, contact details, sensitive constraints, protected traits, and hidden inferences are not shown.
   - `You choose next steps`: decline, not now, pause, report, ask to explore, or share exact details later.
3. **Five-step flow:** use the shared public mental model above.
4. **What this is not:** no autonomous outreach, no private-feed scraping, no hidden people ranking, no production private-overlap computation, no exact contact disclosure from a brief.
5. **Current pilot status:** one compact status card with `available now`, `staff/internal`, `shadow only`, and `not live` labels. Do not list every flag, validator check, or blocker in the default view.
6. **Technical details panel:** move match-signal factor codes, redacted field lists, capability gates, rollout flags, RLS/encryption audit rows, AI shadow contract, source-connector blocker details, DPIA/lawful-basis details, deletion inventory, and route-health contract links into this panel.

Default `/background-networking` copy must avoid showing raw factor-code lists like `cause_area_overlap` in the main explanatory text. Render them as plain labels such as “similar cause area,” “compatible trade type,” and “compatible verification preference,” with exact factor codes available only in technical details.

## `/wish-registry` rewrite instructions

Make `/wish-registry` a broad-preview browsing page, not a private matching explainer. Use this structure:

1. **Hero:** “Browse broad previews. Exact asks stay hidden.”
2. **Search controls:** keep filters minimal by default: keyword/cause, trade mode, and openness. Put advanced filters behind “More filters.”
3. **Preview cards:** each card should show subject kind, broad summary, broad cause tags, broad trade-mode tags, coarse location only where safe, and a clear “Broad preview only” label.
4. **Card actions:** use “View broad profile” and, where signed-in and safe, “Ask to explore.” Do not imply contact, introduction, endorsement, or exact match.
5. **Empty/no-result states:** use side-channel-safe language such as “No privacy-safe broad previews shown for this search. Try a broader search.” Do not imply whether hidden candidates exist.
6. **Technical details:** link to match-signal and private-overlap contracts without rendering JSON or raw contract evidence in the default browsing flow.

## Contract and technical-spec presentation

For public user-facing pages that link to `/api/moral-trade/match-signal/contract`, `/api/moral-trade/private-overlap/contract`, or `/moral-trade/technical-spec`:

- show a compact human summary first: “The contract says a possible opportunity can use only broad preview fields and cannot disclose, contact, rely, or mutate state.”
- show status as `passes current public contract`, `blocked`, `not live`, or `staff/internal`; avoid raw `pass`, `blocker`, or check-count-only copy as the only visible explanation.
- hide raw JSON, route matrices, table names, validator names, exact test command lines, and implementation phrase gates behind “View developer contract.”
- never remove the JSON contracts or technical evidence; simply move them out of the default novice path.
- keep a stable one-click path from every simplified summary to the exact contract route.

## Privacy, safety, measurement, transparency, and accessibility page instructions

Simplify the background-networking sections on trust/legal pages as follows:

- **Privacy:** Replace long inventories in the default view with cards for `Broad preview`, `Private details`, `Source notes`, `Disclosure grants`, `Operator/safety records`, and `Notifications`. Each card must show: what it stores, why, how long, and how to change/delete/export it. Full retention tables and exact inventories move to technical details.
- **Safety:** Start with four plain boundaries: `No surprise exposure`, `No autonomous outreach`, `No private-feed mining`, and `Human review before contact or disclosure`. Keep field-encryption, cache, session, and provider-boundary evidence in technical details.
- **Measurement:** State that background-networking measurement uses aggregate route health, counts, latency buckets, privacy incidents, fallbacks, and review timing; it never stores exact wishes, source notes, contact details, receipt contents, or message text. Keep baseline routes and analytics taxonomy behind technical details.
- **Transparency:** Show an ordinary-user summary of what is counted: possible opportunities delivered/opened, feedback choices, intro packets, disclosure grants, reports, appeals, and review timing. Explain suppression of small counts before showing tables. Move source-table names, sample-size implementation notes, and live schema-cache errors behind technical details or operator/debug surfaces.
- **Accessibility:** Provide a short participant-facing checklist for keyboard, screen-reader, mobile, loading, forms, opportunity inbox, consent dialogs, source-summary review, notification settings, and deletion/freeze flows. Keep the full WCAG/test matrix in technical details.

## Public-page copy rules

Default public Background Networking copy must:

- use “possible opportunity,” not “match,” where the user might otherwise infer endorsement, introduction, ranking, or contact.
- use “broad preview,” not “public compatibility signal,” in default copy.
- use “ask to explore,” not “request counterparty” or “create intro packet,” in default copy.
- use “share exact details,” not “field-level disclosure grant,” in default copy.
- use “pause everything now,” not “privacy freeze,” as the primary emergency-control label, with “privacy freeze” only in technical details.
- explain “manual source notes” as “notes you choose to add,” and repeat that raw feeds are not continuously searched.
- avoid “deterministic,” “anti-enumeration,” “RLS,” “DPIA,” “validator,” “contract check,” “blocker,” “bundle,” “manifest,” “policy decision,” “candidate handle,” “artifact,” and raw factor-code strings in default public copy.
- avoid reassurance-only claims such as “private by design” unless the same page says what remains hidden, what can still be stored, what can be revoked, and what may remain as redacted/anonymized audit records.
- avoid urgency, scarcity, popularity, conversion, or fear-of-missing-out language.

## Public-page copy budgets

For each simplified public page, the default view should satisfy these limits unless accessibility/legal review explicitly requires a documented exception:

- maximum one primary H1 and one primary CTA above the fold;
- maximum three safety cards before any technical details;
- maximum five default flow steps;
- maximum eight default status labels;
- maximum one visible contract/status block before a technical-details expansion;
- no raw JSON, raw test command, raw route matrix, or table/schema name in the default novice path;
- all default sections must have a clear user question as the heading or subheading, such as “What can it use?”, “What stays hidden?”, “How do I stop it?”, or “What happens next?”.

## Feature-preservation requirements

Simplification must not compromise any part of any feature. A simplified public page must still visibly disclose and link to:

- broad previews before exact private details;
- separate outbound “find opportunities for me” authorization and inbound “let others find me” exposure;
- no autonomous outreach;
- no raw private-feed ingestion or continuous private-feed mining;
- no hidden ML ranking of people;
- no exact wishes, contact details, sensitive constraints, raw source notes, protected traits, ideology/psychology inferences, or private third-party data in ordinary opportunity briefs;
- human/operator review before disclosure, contact, reliance, or counterparty-facing state changes;
- ability to decline, not now, report, pause, revoke, freeze, delete background-networking data, export data, and request correction;
- privacy-safe aggregate reporting and small-count suppression;
- links to privacy, safety, accessibility, transparency, match-signal contract, private-overlap contract, and technical spec.

If a simplified card cannot fit one of these disclosures safely, the route must show a concise card plus a technical-details disclosure, not omit the disclosure.
...
## `background-public-page-simplification.test.ts`

Verify:

- `/background-networking` uses the simplified hero, three safety cards, five-step flow, compact pilot status, and technical-detail panel; it does not show raw factor-code strings, raw flag dumps, validator check lists, or JSON in the default novice path
- `/wish-registry` defaults to broad-preview browsing with minimal filters, broad-preview-only card labels, side-channel-safe empty states, and no implication of contact, introduction, endorsement, ranking, or exact match
- `/privacy`, `/safety`, `/measurement`, `/transparency`, and `/accessibility` show concise background-networking summaries first and move full inventories, source-table names, exact route baselines, schema-cache/debug errors, and validator evidence behind technical details
- every simplified public page preserves visible disclosure of broad previews before exact details, separate outbound/inbound consent, no autonomous outreach, no raw private-feed mining, no hidden people ranking, human review before disclosure/contact, and revoke/pause/freeze/delete/export/correction paths
- every simplified public page keeps one-click or equivalent links to the exact match-signal contract, private-overlap contract where relevant, privacy policy, safety policy, accessibility page, transparency report, and technical spec
- public copy uses the active plain-language term map and public-page simplification specs; forbidden default terms, unsafe euphemisms, urgency/scarcity/popularity copy, raw JSON, and unreviewed implementation jargon in the novice path fail promotion
- technical-detail panels remain available, non-disclosing, keyboard/screen-reader accessible, and do not expose candidate-specific gates, hidden blockers, rare-combination internals, private source text, or abuse heuristics
