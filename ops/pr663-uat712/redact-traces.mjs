...
const exactSecrets = [
  ...
  process.env.COMPACT_UAT_PASSWORD,
  process.env.FORBIDDEN_PROD_REF,
...
for (const root of roots) {
  ...
  for (const trace of files.filter((file) =>
    (file.endsWith(".zip") && basename(file).includes("trace")) || /\/uat712-traces\/[^/]+\.zip$/.test(file)
  )) {
    const tracePath = resolve(trace);
    const temp = await mkdtemp(join(tmpdir(), "uat712-trace-"));
    try {
...
for (const root of roots) {
  ...
  for (const file of files) {
    if (file.endsWith(".zip")) {
      const temp = await mkdtemp(join(tmpdir(), "uat712-scan-"));
      try {
