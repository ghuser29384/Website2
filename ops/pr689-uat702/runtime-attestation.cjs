...
module.exports = async function runtimeAttestation(request, response) {
  ...
  const qaServiceRoleConfigured = Boolean(serviceRoleKey);
  let qaServiceRead = false;

  if (qaPublicUrlExact && !productionProjectPresent && qaServiceRoleConfigured) {
    try {
      const serviceRead = await fetch(`${QA_PUBLIC_URL}/auth/v1/admin/users?page=1&per_page=1`, {
        headers: {
