...
begin
  ...
  if prior_observation_id is null then
    raise exception 'The complete-coverage observation required for refund supersession is absent.';
  end if;
  ...
  batch_id := moral_trade_private.compact_outflow_ingest_batch_v1(
    ...
    'hosted-ledger-uat-batch-refund-v2'
  );
  ...
  if result ->> 'status' <> 'recorded' then
    raise exception 'Refund adjustment was not recorded: %', result;
  end if;

  coverage_result := moral_trade_private.freeze_compact_outflow_coverage_v1(
    batch_id,
    'complete',
    'Synthetic hosted-QA partial refund reconciled in a superseding complete snapshot.',
    '2026-08-21T00:00:00Z',
    pg_catalog.jsonb_build_array(
      pg_catalog.jsonb_build_object(
        'adapter_key', 'qa_authoritative_synthetic',
        'disposition', 'complete',
        'source_watermark', 'hosted-ledger-watermark-refund-v2',
        'evidence_hash', 'sha256:' || repeat('c', 64)
      )
    ),
    'hosted-ledger-coverage-refund-v2'
  );
  ...
  if coverage_id is null
    ...
    raise exception 'Refund coverage did not freeze exactly: %', coverage_result;
  end if;
  ...
  if result ->> 'authorityStatus' <> 'complete'
     ...
     or (result ->> 'eligibleNetSettledOutflowCents')::bigint <> 10000
     or (result ->> 'obligationCents')::bigint <> 1000
     or coalesce((result ->> 'moneyMoved')::boolean, true)
     or coalesce((result ->> 'paymentMandateCreated')::boolean, true) then
    raise exception 'Refund supersession did not yield the exact shadow result: %', result;
  end if;
  ...
  if obligation.id is null
     ...
     or obligation.eligible_net_settled_outflow_cents <> 10000
     or obligation.obligation_cents <> 1000
     or obligation.supersedes_id is null then
    raise exception 'Refund obligation did not preserve explicit supersession.';
  end if;
end;
$fixture$;
...
select 'hosted_refund_active_observation|' || count(*)
from public.mpgf_public_goods_outflow_observations observation
join moral_trade_private.compact_outflow_event_metadata event_meta
  on event_meta.observation_id = observation.id
where observation.participant_id = '712a0000-0000-4000-8000-000000000001'::uuid
  and observation.source_record_key = 'hosted-ledger-payment-a'
  and event_meta.source_sequence = 2
  and not exists (
    select 1
    from moral_trade_private.compact_outflow_event_metadata successor
    where successor.supersedes_observation_id = observation.id
...
select 'hosted_refund_shadow_obligation|' || count(*)
...
where participant_id = '712a0000-0000-4000-8000-000000000001'::uuid
  ...
  and eligible_net_settled_outflow_cents = 10000
  and obligation_cents = 1000
