(function enhanceTradeBuilderWithAuthenticatedFeed() {
  ...
  function normalizeRecommendation(value) {
    ...
    const opportunityType = ALLOWED_TYPES.has(value.opportunityType)
    ...
    return {
      ...
      opportunityType,
      exposureRequestId: text(value.exposureRequestId, 160),
  ...
  function readModel() {
    ...
    if (status === "ready" && recommendations.length === 0) status = "no_matches";
  ...
  function recommendationCard(recommendation, index) {
    ...
    const receiptAttribute = recommendation.exposureRequestId
  ...
  function renderRoot(root) {
    ...
    const signature = `${model.status}:${model.requestId}:${model.recommendations
    ...
    if (model.status === "ready" && model.recommendations.length > 0) {
      root.innerHTML = model.recommendations
