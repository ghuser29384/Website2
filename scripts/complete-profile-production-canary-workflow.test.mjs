...
test("pull-request checks cannot cancel the production canary", async () => {
  const source = await workflow();
...
test("current alias ownership selects the exact production deployment", async () => {
  const source = await workflow();

  assert.match(source, /api\.vercel\.com\/v4\/aliases\/\$\{alias_name\}/);
  assert.match(source, /\.deploymentId \/\/ \.deployment\.id \/\/ \.deployment\.uid/);
  assert.match(source, /\$apex_deployment_id" != "\$canonical_deployment_id/);
  ...
  assert.match(source, /Reconfirm the exact deployment still serves both production aliases/);
...
test("the six rendered scenarios and alias reconfirmation precede success", async () => {
  const source = await workflow();
  ...
  const reconfirmIndex = source.indexOf(
    "- name: Reconfirm the exact deployment still serves both production aliases",
  );
