...
2. The participant creates a **structured wish profile** through explicit fields and, where enabled, a fluent but schema-bound interview.
3. The participant may add **manual or imported source summaries**, but raw feeds are not continuously searched, stored in analytics, or used directly for matching.
4. The system converts only user-approved wishes and source summaries into **broad matching signals**:
   - cause areas
   - offers
   - asks
   - capabilities
   ...
   - privacy stage
   - exclusions
5. A background helper job periodically searches the wish registry using only those broad signals.
6. If it finds a high-confidence broad match, it creates an **opportunity brief** with:
   - reason codes
   - confidence band
   - visible redactions
   - blockers
   - next-step options
7. Notifications are generic. They never include exact wishes, source notes, contact details, private constraints, or counterparty-specific sensitive content.
8. The participant can dismiss, defer, report, or request an introduction.
9. Only after mutual consent and operator review can field-level grants disclose more detail.

This matches Forethought’s “helpers in the background + wish profiling + semi-private registry” design while preserving Moral Trade’s safety posture.
...
2. Participant creates or updates a structured wish profile through explicit fields and, where enabled, a schema-bound fluent interview.
3. Participant may add a manual or imported source summary. Raw private feeds must not be continuously ingested, searched, copied into analytics, or used directly for matching.
4. Only participant-approved broad signals become match inputs.
5. A deterministic background helper run searches broad wish-registry previews and approved broad profile signals.
6. If a candidate match passes eligibility and threshold checks, create a privacy-safe opportunity brief.
7. Notify the participant with generic copy only.
8. Participant can dismiss, defer, report, or request an intro.
9. Intro requests remain operator-reviewed and mutual-consent gated before any exact wishes, contact details, sensitive constraints, source notes, or private counterparties are disclosed.
...
Production defaults must be `false` unless the repository already has a staged rollout convention that says otherwise. Tests may enable the flags explicitly.
...
- Do not trigger live matching unless `BACKGROUND_DELEGATE_RUNS_ENABLED` is true and the user explicitly opted in.
...
- Delegate runs generate deterministic privacy-safe opportunity briefs.
...
Forethought itself treats the privacy/surveillance/collusion tradeoff as unresolved and important. The safer concrete mechanism is therefore to generate reviewable opportunity briefs from approved broad signals rather than expose richer hidden matching infrastructure immediately.
