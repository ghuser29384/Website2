...
export function evaluateDeterministicMatch({
  ...
  const basis = [
    ...
    askOfferOverlap.length ? `Possible complementarity: ${askOfferOverlap.join(", ")}` : "",
    sharedTokens.length ? `Shared language: ${sharedTokens.join(", ")}` : "",
    ...
    geographicOverlap.join(" / "),
    participantComplement.join(" / "),
